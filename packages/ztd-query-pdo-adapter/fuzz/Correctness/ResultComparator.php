...
final class ResultComparator
{
    ...
    public function compareValue(mixed $expected, mixed $actual, string $type = '', string $column = ''): bool
    {
        ...
        $expectedStr = is_scalar($expected) ? (string) $expected : '';
        $actualStr = is_scalar($actual) ? (string) $actual : '';

        if (str_contains($type, 'FLOAT') || str_contains($type, 'DOUBLE')) {
            return $this->compareFloat((float) $expectedStr, (float) $actualStr);
        }

        if (str_contains($type, 'DECIMAL') || str_contains($type, 'NUMERIC')) {
            return $this->compareDecimal($expectedStr, $actualStr);
        }

        if ($type === 'JSON') {
            return $this->compareJson($expectedStr, $actualStr);
        }
