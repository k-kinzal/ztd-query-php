...
final class StatementExecutionTest extends TestCase
{
    ...
    public function testResultUpdatesShadowAndReturnsItsResult(): void
    {
        ...
        $native->exec('CREATE TABLE items (id INTEGER PRIMARY KEY, value TEXT)');
        $executor = new \ZtdQuery\QueryExecutor(new PdoConnection($native), new SqlitePlatform(), ZtdConfig::default());
        $query = new PreparedQuery($executor, 'INSERT INTO items VALUES (?, ?)', static fn (string $sql): PDOStatement|false => $native->prepare($sql));
        $plan = $query->rewrite();
        $execution = new StatementExecution($query->prepare($plan->sql()), $executor, $plan, $query);
        ...
        $shadow = $native->query($executor->rewrite('SELECT COUNT(*) AS total FROM items')->sql());
    ...
    public function testNativeUnrewrittenExecutionDelegatesToTheNativeStatement(): void
    {
        ...
        $execution = new StatementExecution($statement, $executor, null);
    ...
    public function testExecuteSilentFailureDoesNotProduceAShadowResult(): void
    {
        ...
        $executor = new \ZtdQuery\QueryExecutor(new PdoConnection($native), new SqlitePlatform(), ZtdConfig::default());
        $execution = new StatementExecution($native->prepare('INSERT INTO items VALUES (1)'), $executor, null);
    ...
    public function testRowCountTracksNativeMutations(): void
    {
        ...
        $write = new StatementExecution($insert, $executor, null);
    ...
    public function testPostProcessAppliesResultSelectRowsToTheShadow(): void
    {
        ...
        $executor = new \ZtdQuery\QueryExecutor(new PdoConnection($native), new SqlitePlatform(), ZtdConfig::default());
        $plan = $executor->rewrite("INSERT INTO users VALUES (1, 'Ada')");
        ...
        self::assertNotNull($execution->result());
        self::assertSame(1, $execution->result()->rowCount());
        $read = $native->query($executor->rewrite('SELECT name FROM users')->sql());
    ...
    public function testBindParameterRetainsReferencesAcrossRepreparation(): void
    {
        ...
        self::assertNotFalse($statement);
        $prepared = new PreparedQuery($executor, 'SELECT :id AS id', $native->prepare(...));
        $execution = new StatementExecution($statement, $executor, null, $prepared);
        $id = 7;
