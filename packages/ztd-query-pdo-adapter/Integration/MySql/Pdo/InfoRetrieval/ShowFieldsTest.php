...
final class ShowFieldsTest extends MySqlIntegrationTestCase
{
    public function testShowFieldsThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("SHOW FIELDS FROM `{$table}`");
    }

    public function testShowFieldsIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("SHOW FIELDS FROM `{$table}`");

        $this->assertFalse($result);
    }

    public function testShowFieldsTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('SHOW FIELDS', $capturedNotices[0]);
        } finally {
