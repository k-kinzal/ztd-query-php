...
final class ExplainAnalyzeTest extends MySqlIntegrationTestCase
{
    public function testExplainAnalyzeThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("EXPLAIN ANALYZE SELECT * FROM `{$table}`");
    }

    public function testExplainAnalyzeIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("EXPLAIN ANALYZE SELECT * FROM `{$table}`");

        $this->assertFalse($result);
    }

    public function testExplainAnalyzeTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('EXPLAIN ANALYZE', $capturedNotices[0]);
        } finally {
