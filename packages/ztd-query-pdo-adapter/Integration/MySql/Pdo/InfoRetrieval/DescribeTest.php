...
final class DescribeTest extends MySqlIntegrationTestCase
{
    public function testDescribeThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("DESCRIBE `{$table}`");
    }

    public function testDescribeIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("DESCRIBE `{$table}`");

        $this->assertFalse($result);
    }

    public function testDescribeTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('DESCRIBE', $capturedNotices[0]);
        } finally {
