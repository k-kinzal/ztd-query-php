...
final class HandlerCloseTest extends MySqlIntegrationTestCase
{
    public function testHandlerCloseThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("HANDLER `{$table}` CLOSE");
    }

    public function testHandlerCloseIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->exec("HANDLER `{$table}` CLOSE");

        $this->assertSame(0, $result);
    }

    public function testHandlerCloseTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
