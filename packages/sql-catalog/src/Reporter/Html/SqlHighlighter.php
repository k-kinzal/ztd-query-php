...
final class SqlHighlighter
{
     ...
     */
    private const KEYWORDS = 'SELECT|INSERT|UPDATE|DELETE|REPLACE|MERGE|TRUNCATE|CREATE|ALTER|DROP|RENAME|CALL|SHOW|EXPLAIN|DESCRIBE|WITH|RECURSIVE|FROM|WHERE|GROUP|BY|HAVING|ORDER|LIMIT|OFFSET|FETCH|JOIN|INNER|LEFT|RIGHT|FULL|OUTER|CROSS|NATURAL|LATERAL|ON|USING|UNION|INTERSECT|EXCEPT|ALL|DISTINCT|AS|INTO|VALUES|SET|DEFAULT|RETURNING|AND|OR|NOT|IN|IS|NULL|LIKE|ILIKE|RLIKE|REGEXP|BETWEEN|EXISTS|CASE|WHEN|THEN|ELSE|END|ASC|DESC|NULLS|FIRST|LAST|TABLE|VIEW|INDEX|DATABASE|SCHEMA|COLUMN|CONSTRAINT|PRIMARY|FOREIGN|KEY|UNIQUE|REFERENCES|CASCADE|RESTRICT|IF|DUPLICATE|IGNORE|STRAIGHT_JOIN|SQL_CALC_FOUND_ROWS|FOR|SHARE|UPDATE|LOCK|BEGIN|START|TRANSACTION|COMMIT|ROLLBACK|SAVEPOINT|GRANT|REVOKE|ANALYZE|OPTIMIZE|VACUUM|PRAGMA|CAST|CONVERT|COLLATE|INTERVAL|PARTITION|OVER|WINDOW|FILTER|GROUPS|RANGE|ROWS|UNBOUNDED|PRECEDING|FOLLOWING|CURRENT|ROW|TRUE|FALSE|UNSIGNED|BINARY|CHARACTER|CHARSET';

    /**
     * How the tokens of a resolved run are told apart.
     */
    private const TOKENS = '/(?<com>--[^\n]*|\#[^\n]*|\/\*.*?\*\/)|(?<str>\'(?:\'\'|\\\\.|[^\'])*\'|"(?:""|\\\\.|[^"])*")|(?<qid>`[^`]*`)|(?<ph>\?|:[A-Za-z_][A-Za-z0-9_]*|\$[0-9]+)|(?<num>\b[0-9]+(?:\.[0-9]+)?\b)|(?<word>[A-Za-z_][A-Za-z0-9_]*)/s';
