...
final class SelectCompilerTest extends TestCase
{
    ...
    public function testCompilePreservesExplicitProjectionAndWrapsExists(): void
    {
        ...
        self::assertSame('select `id` from `users` where 0 = 1', $compiler->compile($state->with('key', Domain::literal('id')), 'find', [QueryState::list([])])[0]->soleLiteral()?->value);
    }
    ...
    public function testKeyComparesScalarsAndListsAgainstThePrimaryKey(): void
    {
        ...
        self::assertSame('"users"."id" in (?, ?)', $many->items('where')[0]->soleLiteral()?->value);
    ...
    public function testTotalCountsWithoutTheWindowOrOrdering(): void
    {
        ...
        self::assertFalse($compiler->total($state->with('groups', QueryState::list([Domain::literal('"a"')])))[0]->isExact());
        self::assertFalse($compiler->total($state->with('distinct', Domain::literal(true)))[0]->isExact());
    }
