    ...
    public function testRenderIsWrittenExactly(): void
    {
        ...
        self::assertSame(
            '<h1><code>lib/c.php</code><span class="count">2 statements</span></h1><p class="lede">2 statements issued from 2 functions in this file.</p>'
                . '<h2 id="tables">Tables</h2><p class="empty-inline">No statement here names a table.</p><h2 id="functions">Functions</h2><div data-narrowable'
                . '><div class="facets" role="group" aria-label="Narrow the listing"><input type="search" name="narrow" class="input facet-search" placeholder='
                . '"Narrow by text…" aria-label="Narrow by text" autocomplete="off" spellcheck="false"><span class="facet-group"><button type="button" class='
                . '"chip facet tone-blue" data-facet="kind" data-value="select" aria-pressed="false">SELECT<span class="facet-count">1</span></button><button t'
                . 'ype="button" class="chip facet tone-slate" data-facet="kind" data-value="unknown" aria-pressed="false">UNKNOWN<span class="facet-count">1</s'
                . 'pan></button></span><span class="facet-group"><button type="button" class="chip facet chip-ghost" data-facet="resolution" data-value="incomp'
                . 'lete" aria-pressed="false">incomplete<span class="facet-count">1</span></button><button type="button" class="chip facet tone-neutral" data-f'
                . 'acet="resolution" data-value="not-analyzed" aria-pressed="false">not-analyzed<span class="facet-count">1</span></button></span><span class="'
                . 'facet-shown" aria-live="polite"></span><button type="button" class="btn facet-clear" hidden>Clear</button></div><section class="group" id="f'
                . 'n-helper"><h3><code>helper</code><span class="count">1</span><span class="muted">line 2</span><a class="anchor" href="#fn-helper">#</a></h3>'
                . '<ol class="rows"><li class="row" data-kind="select" data-resolution="incomplete" data-severity="low" data-rule="analysis-incomplete" data-si'
                . 'nk="mysqli.query" data-open="open" data-table="" data-namespace="" data-class="" data-function="helper" data-file="lib/c.php"><a class="row-'
                . 'main" href="../statements/c1.html"><span class="chip tone-blue">SELECT</span><pre class="code"><span class="hole tone-warn" title="This is a'
                . ' gap: a dependency the analyzer stopped following fills it.">{$}</span></pre></a><p class="row-meta"><span class="chip chip-ghost" title="A '
                . 'cycle or an analysis budget stopped the search before it closed.">incomplete</span></p></li></ol></section><section class="group" id="fn-mai'
                . 'n"><h3><code>top-level code</code><span class="count">1</span><span class="muted">line 6</span><a class="anchor" href="#fn-main">#</a></h3><'
                . 'ol class="rows"><li class="row" data-kind="unknown" data-resolution="not-analyzed" data-severity="low" data-rule="call-not-analyzed" data-si'
                . 'nk="unmatched" data-open="open" data-table="" data-namespace="" data-class="" data-function="{main}" data-file="lib/c.php"><a class="row-mai'
                . 'n" href="../statements/c2.html"><span class="chip tone-slate">UNKNOWN</span><pre class="code"><span class="tok-com">no statement was read fr'
                . 'om this call</span> $db-&gt;query($sql)</pre></a><p class="row-meta"><span class="chip tone-neutral" title="The call was found but never exa'
                . 'mined, so nothing was read from it.">not-analyzed</span></p></li></ol></section></div>',
