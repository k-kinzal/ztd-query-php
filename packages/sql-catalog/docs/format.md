...
| `statements[].site` | The file and line, the enclosing function, and which database call was matched. A `sink` of `unreached` means the call was found but the walk never got to it; `unmatched` means the walk got to it but could not tell what it was called on. Both carry a `not-analyzed` resolution, which is how a gap in the analysis is told apart from a statement whose text did not resolve. |
