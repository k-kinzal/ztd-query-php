...
#[CoversClass(\ZtdQuery\Platform\Postgres\Reflection\Key\UniqueIndexes::class)]
...
final class UniqueIndexesTest extends TestCase
{
    ...
    {
        $statement = new \Tests\Fake\FakeStatement([['constraint_name' => 'active_email', 'column_name' => 'email', 'predicate' => "status = 'active'"]]);
        $indexes = (new \ZtdQuery\Platform\Postgres\Reflection\Key\UniqueIndexes())->definitions($statement);
        self::assertSame([], $indexes->sql);
        self::assertSame("status = 'active'", $indexes->partialIndexes['active_email']->predicate);
        self::assertSame([], (new \ZtdQuery\Platform\Postgres\Reflection\Key\UniqueIndexes())->definitions(false)->partialIndexes);
    }
}
