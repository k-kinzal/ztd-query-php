...
final class QuotedInputTest extends TestCase
{
    ...
    {
        $input = new \ZtdQuery\Platform\Postgres\Connection\Parameter\QuotedInput();
        $cursor = new \ZtdQuery\Platform\Postgres\Connection\Parameter\EscapeCursor('/* ? /* nested */ ? */');
        ...
        self::assertFalse($input->consumeIgnored(new \ZtdQuery\Platform\Postgres\Connection\Parameter\EscapeCursor('x')));
    }

    public function testConsumeQuotedPreservesLiteralQuestionMarks(): void
    {
        $input = new \ZtdQuery\Platform\Postgres\Connection\Parameter\QuotedInput();
        $cursor = new \ZtdQuery\Platform\Postgres\Connection\Parameter\EscapeCursor("'a''?b' tail");
        self::assertTrue($input->consumeQuoted($cursor));
        self::assertSame("'a''?b'", $cursor->result);
        self::assertFalse($cursor->expectsOperand);
        $dollar = new \ZtdQuery\Platform\Postgres\Connection\Parameter\EscapeCursor('$tag$?$tag$');
        self::assertTrue($input->consumeQuoted($dollar));
        self::assertSame('$tag$?$tag$', $dollar->result);
        self::assertFalse($input->consumeQuoted(new \ZtdQuery\Platform\Postgres\Connection\Parameter\EscapeCursor('$1')));
    }
}
