...
#[CoversClass(\ZtdQuery\Platform\Postgres\Parsing\Expression\PrecedenceParser::class)]
...
final class PrecedenceParserTest extends TestCase
{
    public function testParseOrPreservesPrecedenceAndConsumesItsTokens(): void
    {
        ...
        $expression = (new \ZtdQuery\Platform\Postgres\Parsing\Expression\PrecedenceParser($cursor))->parseOr();
    ...
    public function testParseAndPreservesPrecedenceAndConsumesItsTokens(): void
    {
        ...
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Postgres\PgSqlLexerProfile::create())->significantTokens();
        $cursor = new \ZtdQuery\Platform\Postgres\Parsing\Expression\ExpressionCursor($sql, 'users', $tokens);
        $expression = (new \ZtdQuery\Platform\Postgres\Parsing\Expression\PrecedenceParser($cursor))->parseAnd();
        self::assertSame(true, $expression->evaluate([], [], 'users'));
        self::assertSame(count($tokens), $cursor->index);
    }
    ...
    public function testParseComparisonPreservesPrecedenceAndConsumesItsTokens(): void
    {
        ...
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Postgres\PgSqlLexerProfile::create())->significantTokens();
        $cursor = new \ZtdQuery\Platform\Postgres\Parsing\Expression\ExpressionCursor($sql, 'users', $tokens);
        $expression = (new \ZtdQuery\Platform\Postgres\Parsing\Expression\PrecedenceParser($cursor))->parseComparison();
        self::assertSame(true, $expression->evaluate([], [], 'users'));
        self::assertSame(count($tokens), $cursor->index);
    }
    ...
    public function testParseAdditivePreservesPrecedenceAndConsumesItsTokens(): void
    {
        ...
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Postgres\PgSqlLexerProfile::create())->significantTokens();
        $cursor = new \ZtdQuery\Platform\Postgres\Parsing\Expression\ExpressionCursor($sql, 'users', $tokens);
        $expression = (new \ZtdQuery\Platform\Postgres\Parsing\Expression\PrecedenceParser($cursor))->parseAdditive();
    ...
    public function testParseMultiplicativePreservesPrecedenceAndConsumesItsTokens(): void
    {
        ...
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Postgres\PgSqlLexerProfile::create())->significantTokens();
        $cursor = new \ZtdQuery\Platform\Postgres\Parsing\Expression\ExpressionCursor($sql, 'users', $tokens);
        $expression = (new \ZtdQuery\Platform\Postgres\Parsing\Expression\PrecedenceParser($cursor))->parseMultiplicative();
    ...
    public function testParseUnaryPreservesPrecedenceAndConsumesItsTokens(): void
    {
        ...
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Postgres\PgSqlLexerProfile::create())->significantTokens();
        $cursor = new \ZtdQuery\Platform\Postgres\Parsing\Expression\ExpressionCursor($sql, 'users', $tokens);
        $expression = (new \ZtdQuery\Platform\Postgres\Parsing\Expression\PrecedenceParser($cursor))->parseUnary();
