...
#[CoversClass(\ZtdQuery\Platform\Postgres\Driver\Placeholder\OperandInput::class)]
...
final class OperandInputTest extends TestCase
{
    ...
    {
        $input = new \ZtdQuery\Platform\Postgres\Driver\Placeholder\OperandInput();
        $cursor = new \ZtdQuery\Platform\Postgres\Driver\Placeholder\EscapeCursor('? ?|');
        ...
        self::assertFalse($input->consumeOperator(new \ZtdQuery\Platform\Postgres\Driver\Placeholder\EscapeCursor('word')));
    }
    ...
    public function testConsumeOperandTracksKeywordsAndNamedParameters(): void
    {
        ...
        $parameter = new \ZtdQuery\Platform\Postgres\Driver\Placeholder\EscapeCursor(':select');
        $parameter->preserve(1);
        self::assertTrue($input->consumeOperand($parameter));
        self::assertFalse($parameter->expectsOperand);
        $number = new \ZtdQuery\Platform\Postgres\Driver\Placeholder\EscapeCursor('12.5');
