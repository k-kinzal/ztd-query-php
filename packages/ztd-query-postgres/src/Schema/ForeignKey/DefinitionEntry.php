...
namespace ZtdQuery\Platform\Postgres\Schema\ForeignKey;
...
/**
 * Definition entry operations for PostgreSQL foreignkey.
 *
 * @visibility root
 */
final class DefinitionEntry
{
    ...
    public function parseEntry(
        ...
        $references = DefinitionTokens::keywordIndex($tokens, 'REFERENCES');
        if ($references === null) {
            return null;
        }
    ...
    public function foreignKeyColumns(
        ...
        $foreign = DefinitionTokens::keywordIndex(array_slice($tokens, 0, $references), 'FOREIGN');
        if ($foreign === null) {
            return [];
        }
    ...
    public function referencedRelation(
        ...
        while (DefinitionTokens::isSymbol($tokens[$next] ?? null, '.')) {
        ...
        $opening = DefinitionTokens::symbolIndex($tokens, '(', $next);
    ...
    public function identifierList(SqlTokenStream $stream, array $tokens, int $opening): array
    {
        ...
        foreach ($candidates as $token) {
            ...
            if ($token->depth === $depth) {
                if (DefinitionTokens::isSymbol($token, ')')) {
                    return $identifiers;
                }
