...
namespace ZtdQuery\Platform\Postgres\Mutation;
...
final class DmlResolver
{
    ...
    /**
     * Supplies the dependencies used by this DmlResolver.
     */
    ...
    public function resolveInsert(string $sql): ShadowMutation
    {
        ...
        $storageTable = (new \ZtdQuery\Platform\Postgres\Schema\Partition\StorageTable($this->registry))->storageTable($tableName);
    ...
    public function resolveUpdate(string $sql): ShadowMutation
    {
        ...
        $storageTable = (new \ZtdQuery\Platform\Postgres\Schema\Partition\StorageTable($this->registry))->storageTable($targetTable);
    ...
    public function resolveMerge(string $sql): ShadowMutation
    {
        ...
        return new SynchronizeMutation(
            (new \ZtdQuery\Platform\Postgres\Schema\Partition\StorageTable($this->registry))->storageTable($statement->targetTable),
            $definition,
            $sql,
        );
    }
     ...
     * @throws UnsupportedSqlException
     */
    public function resolveUpsert(string $sql, string $tableName, string $storageTable, array $primaryKeys, array $conflictInfo, ?\ZtdQuery\Schema\CandidateKeySet $candidateKeys, ?string $conflictPredicate): ShadowMutation
    {
        $expressionParser = new PgSqlUpsertExpressionParser();
        $updateColumns = $conflictInfo['columns'];
