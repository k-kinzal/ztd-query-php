...
final class PgSqlTransactionStatementParser implements TransactionStatementParser
{
    ...
    public function parse(string $sql): ?TransactionStatement
    {
        ...
        if ((new Parsing\Transaction\KeywordForm())->matchesAny($tokens, [['BEGIN'], ['BEGIN', 'WORK'], ['BEGIN', 'TRANSACTION'], ['START', 'TRANSACTION']])) {
            return TransactionStatement::begin();
        }
        if ((new Parsing\Transaction\KeywordForm())->matchesAny($tokens, [['COMMIT'], ['COMMIT', 'WORK'], ['COMMIT', 'TRANSACTION'], ['END'], ['END', 'WORK'], ['END', 'TRANSACTION']])) {
            return TransactionStatement::commit();
        }
        if ((new Parsing\Transaction\KeywordForm())->matchesAny($tokens, [['ROLLBACK'], ['ROLLBACK', 'WORK'], ['ROLLBACK', 'TRANSACTION']])) {
            return TransactionStatement::rollback();
        }
        $name = (new Parsing\Transaction\KeywordForm())->nameAfter($tokens, [['SAVEPOINT']]);
        if ($name !== null) {
            return TransactionStatement::savepoint($name);
        }
        $name = (new Parsing\Transaction\KeywordForm())->nameAfter($tokens, [['ROLLBACK', 'TO'], ['ROLLBACK', 'TO', 'SAVEPOINT']]);
        if ($name !== null) {
            return TransactionStatement::rollbackTo($name);
        }
        $name = (new Parsing\Transaction\KeywordForm())->nameAfter($tokens, [['RELEASE'], ['RELEASE', 'SAVEPOINT']]);
