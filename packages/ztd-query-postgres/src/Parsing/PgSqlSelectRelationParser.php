...
/**
 * Select relation parser for PostgreSQL queries.
 */
final class PgSqlSelectRelationParser
{
    ...
    {
        $tokens = (new Parsing\Relation\RelationReference())->tokens($sql);
        /**
        ...
        foreach ($tokens as $token) {
            ...
            $end = (new Parsing\Relation\FromClause())->findFromEnd($sql, $tokens, $token);
    ...
    public function references(string $sql): array
    {
        ...
        foreach ($tokens as $token) {
            ...
            $clauseEnd = (new Parsing\Relation\FromClause())->findFromEnd($sql, $tokens, $token);
            $clause = substr($sql, $clauseStart, $clauseEnd - $clauseStart);
            foreach ((new Parsing\Relation\FromClause())->referencesFromClause($clause) as $reference) {
                $references[] = [
                    'name' => $reference['name'],
