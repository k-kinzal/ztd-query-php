...
final class PgSqlConflictTarget
{
    ...
    public function resolve(CandidateKeySet $candidateKeys, array $partialIndexes, string $sql): array
    {
        ...
        $targetColumns = Parsing\Conflict\ColumnSet::normalizedColumns($this->columns);
        $fullMatches = [];
        foreach ($candidateKeys->keys() as $name => $columns) {
            if (Parsing\Conflict\ColumnSet::normalizedColumns($columns) === $targetColumns) {
        ...
        foreach ($partialIndexes as $name => $index) {
            if (Parsing\Conflict\ColumnSet::normalizedColumns($index->columns) === $targetColumns) {
