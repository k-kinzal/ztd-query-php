...
final class PgSqlValueRenderer implements ValueRenderer
{
     ...
     *     $renderer = new \ZtdQuery\Platform\Postgres\PgSqlValueRenderer();
    ...
    public function renderValue(mixed $value, ?ColumnType $type = null): string
    {
        ...
        $text = new Sql\Value\LiteralText();
        if ($type === null) {
            ...
            $inferred = new ColumnType(is_int($value) ? ColumnTypeFamily::INTEGER : ColumnTypeFamily::TEXT, is_int($value) ? 'INTEGER' : 'TEXT');
        ...
        if ($type->family === ColumnTypeFamily::BINARY) {
            if (is_resource($value) && get_resource_type($value) === 'stream') {
                $bytes = (new Sql\Value\BinaryStream())->read($value);
