...
final class PgSqlSelectRelationParser
{
    ...
    {
        $tokens = (new RelationReference())->tokens($sql);
        /**
        ...
        foreach ($tokens as $token) {
            ...
            $end = (new FromClause())->findFromEnd($sql, $tokens, $token);
    ...
    public function references(string $sql): array
    {
        ...
        foreach ($tokens as $token) {
            ...
            $clauseEnd = (new FromClause())->findFromEnd($sql, $tokens, $token);
            $clause = substr($sql, $clauseStart, $clauseEnd - $clauseStart);
            foreach ((new FromClause())->referencesFromClause($clause) as $reference) {
                $references[] = [
                    'name' => $reference['name'],
