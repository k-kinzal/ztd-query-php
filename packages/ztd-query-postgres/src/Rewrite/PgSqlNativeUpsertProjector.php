...
final class PgSqlNativeUpsertProjector
{
    ...
    public function project(
        ...
        $conflict = (new Rewrite\Upsert\ConflictPredicate($this->quoter))->conflictPredicate($candidateKeys, $existingAlias, $incomingAlias);
        if ($conflictPredicate !== null) {
            $conflict = (new Rewrite\Upsert\ConflictPredicate($this->quoter))->withPredicate($conflict, $conflictPredicate, $tableName, $tableColumns, $this->incomingNamespaces);
        }
        $selects = [];
        ...
        foreach (array_values($assignments) as $index => $expression) {
            $evaluated = (new Rewrite\Upsert\ExpressionBinder($this->incomingNamespaces, $this->quoter))->bindExpression($expression, $tableName, $tableColumns);
        ...
        if ($predicate !== null) {
            $evaluated = (new Rewrite\Upsert\ExpressionBinder($this->incomingNamespaces, $this->quoter))->bindExpression($predicate, $tableName, $tableColumns);
