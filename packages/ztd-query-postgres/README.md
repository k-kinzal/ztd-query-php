...
final class UserQueryTest extends TestCase
{
    ...
    {
        $pdo = new ZtdPdo('pgsql:host=127.0.0.1;dbname=test', 'test', 'test');
...
| Upsert | `ON CONFLICT` |
| UPDATE and DELETE | Supported, including `UPDATE ... FROM` and `DELETE ... USING` |
| `RETURNING` | Supported |
| TRUNCATE | Supported |
