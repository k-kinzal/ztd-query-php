<?php

declare(strict_types=1);

namespace Tests\Unit;

use Container\MySql91Container;
use Container\MySqlConfiguration;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\UsesClass;
use PHPUnit\Framework\TestCase;

#[CoversClass(MySql91Container::class)]
#[UsesClass(MySqlConfiguration::class)]
final class MySql91ContainerTest extends TestCase
{
    /**
     * @throws \Testcontainers\Exceptions\InvalidFormatException
     */
    public function testImagePinsTheReusableServer(): void
    {
        $container = new MySql91Container();
        self::assertSame('container-registry.oracle.com/mysql/community-server:9.1.0', $container->image());
        self::assertTrue($container->reuseMode()->isReuse());
    }

    public function testGetGrammarVersionMatchesTheServer(): void
    {
        self::assertSame('mysql-9.1.0', MySql91Container::getGrammarVersion());
    }
}
