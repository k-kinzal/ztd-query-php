...
final class ParseBench
{
    ...
    /**
     * Builds a grammar of two hundred rules with actions, aliases, precedence and a conditional region before measurement.
     */
    public function setUp(): void
    {
        $lines = ['%token_prefix TK_', '%left PLUS MINUS.', '%left STAR SLASH.', '%ifndef OMIT_EXTRA'];
        for ($index = 0; $index < 200; $index++) {
            $lines[] = "rule{$index}(A) ::= rule{$index}(B) PLUS|MINUS NUM(C). [STAR] { A = B + C; }";
        }
        $lines[] = '%endif';
        $this->source = implode("\n", $lines) . "\n";
