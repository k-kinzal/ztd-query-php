# Fuzzing lemon-parser

`composer fuzz:parse` and `composer fuzz:roundtrip` run one PHP-Fuzzer target each.
Both feed arbitrary text to the parser, guided by a dictionary of Lemon tokens. A first line
starting with `%D` names the defines to preprocess with, so the fuzzer reaches `%ifdef` regions.
