...
final class TextFragment
{
    ...
    public static function text(string $fragment): string
    {
        if (preg_match('/\A#[^#]*?:~:text=([^,&\-]+)\z/u', $fragment, $match) !== 1 || preg_match('/%(?![0-9a-f]{2})/i', $fragment) === 1) {
            throw new InvalidArgumentException('Use one exact Text Fragment (#:~:text=percent-encoded-text); ranges, context terms and multiple directives are unsupported.');
        }
        $text = rawurldecode($match[1]);
        if (!mb_check_encoding($text, 'UTF-8') || Unit::normalize($text) === '') {
            throw new InvalidArgumentException('Text Fragment text must be nonempty UTF-8.');
        }
