...
final class Document
{
     ...
     */
    public function __construct(Node $tree, Settings $settings)
    {
        $trivia = new Trivia($settings->nesting);
        $cleaned = [];
        $protected = [];
        $verbatim = [];
        $trailing = '';
        ...
        $tokens = (new Visitor($cleaned, $protected, $verbatim, $settings))->tokens($tree);
        ...
        $this->tokens = $tokens;
        $this->candidates = (new Reductions($settings->grouping, $settings->aliases))->candidates($tree, $protected);
