...
use SqlFormatter\Core\Compact\Document;
use SqlFormatter\Core\Compact\Grouping;
use SqlFormatter\Core\Compact\Keywords;
use SqlFormatter\Core\Compact\Renderer;
use SqlFormatter\Core\Compact\Rules;
use SqlFormatter\Core\Compact\Shape;
use SqlFormatter\Core\Compact\Spacing;
use SqlFormatter\Core\Compact\Trivia;
use SqlFormatter\Core\Compact\Visitor;
...
final class RendererTest extends TestCase
{
    ...
    public function testRenderEquivalentSpellings(string $parserClass, ?string $version, string $input, string $expected): void
    {
        ...
        $renderer = new Renderer($parser, \SqlFormatter\Facade\DialectFactory::forParser($parser)->compactRules());
    ...
    {
        $renderer = new Renderer(new SqliteParser(), \SqlFormatter\Facade\DialectFactory::forParser(new SqliteParser())->compactRules());
    ...
    public function testDocumentSelectsDialectSpecificSpellings(): void
    {
        ...
        $document = (new Renderer($parser, \SqlFormatter\Facade\DialectFactory::forParser($parser)->compactRules()))->document($parser->parse('SELECT a FROM t WHERE a REGEXP b'));
