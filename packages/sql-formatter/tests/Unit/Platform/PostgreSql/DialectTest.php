...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlFormatter\Platform\PostgreSql\Dialect::class)]
...
final class DialectTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        self::assertTrue((new \SqlFormatter\Platform\PostgreSql\Dialect())->syntaxRules()->has('clauses', 'where_clause'));
        self::assertFalse((new \SqlFormatter\Platform\PostgreSql\Dialect())->syntaxRules()->has('clauses', 'custom_rule'));
    }

    public function testCompactRulesNormalizeDeclaredTerminalAliases(): void
    {
        $settings = (new \SqlFormatter\Platform\PostgreSql\Dialect())->compactRules();
        self::assertSame('<>', $settings->keywords->text(new \SqlParser\Lexer\Token(1, 'NOT_EQUALS', '!=', 0), false));
    }
}
