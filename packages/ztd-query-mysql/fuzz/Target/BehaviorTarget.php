...
final class BehaviorTarget implements ConnectionInterface
{
    ...
    public function database(string $side): PDO
    {
        ...
        try {
            $root = new PDO($this->dsn, 'root', 'root', $options);
            $root->exec("DROP DATABASE IF EXISTS `$name`");
            $root->exec("CREATE DATABASE `$name`");
            $root->exec("DROP USER IF EXISTS '$name'@'%'");
            $root->exec("CREATE USER '$name'@'%' IDENTIFIED BY '$name'");
            $root->exec("GRANT ALL ON `$name`.* TO '$name'@'%'");
            $pdo = new PDO($this->dsn . ';dbname=' . $name, $name, $name, $options);
            ...
            fwrite(STDERR, "MySQL setup failed: {$failure->getMessage()}\n");
            exit(2);
        }
    }
    ...
    {
        $tables = [];
        $catalog = $pdo->query("SELECT table_name FROM information_schema.tables WHERE table_schema = DATABASE() AND table_type = 'BASE TABLE' ORDER BY table_name");
        if ($catalog === false) {
    ...
    public function snapshot(PDO $pdo): array
    {
        ...
        foreach (array_keys($tables) as $table) {
            $definitions[$table] = $this->native($pdo, 'SHOW CREATE TABLE ' . $this->quote($table));
        }
        return [$tables, $definitions,
            $this->native($pdo, 'SELECT table_name, view_definition FROM information_schema.views WHERE table_schema = DATABASE()'),
            $this->native($pdo, 'SELECT trigger_name, action_statement FROM information_schema.triggers WHERE trigger_schema = DATABASE()'),
            $this->native($pdo, 'SELECT routine_name, routine_definition FROM information_schema.routines WHERE routine_schema = DATABASE()'),
        ];
    }
