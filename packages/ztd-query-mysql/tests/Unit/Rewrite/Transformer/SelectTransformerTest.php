...
use ZtdQuery\Platform\MySql\Rewrite\FullText\MySqlFullTextSearchRewriter;
...
use ZtdQuery\Platform\MySql\Rewrite\Type\MySqlTypeSemantics;
...
#[CoversClass(\ZtdQuery\Platform\MySql\Rewrite\Transformer\Set\OrderRewriter::class)]
#[CoversClass(\ZtdQuery\Platform\MySql\Rewrite\Transformer\Set\ValueNormalizer::class)]
#[CoversClass(\ZtdQuery\Platform\MySql\Rewrite\Transformer\Shadow\CteRows::class)]
final class SelectTransformerTest extends TestCase
{
    ...
    public function testCteInjectedSqlStartsWithWith(): void
    {
        $transformer = new SelectTransformer();
        $sql = 'SELECT * FROM users WHERE id = 1';
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice', 'email' => 'alice@example.com']], 'columns' => ['id', 'name', 'email'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INT'), 'name' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'VARCHAR(255)'), 'email' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'VARCHAR(255)')]]];
        $result = $transformer->transform($sql, $tables);
    ...
    public function testCteContainsSelectUnionStructure(): void
    {
        $transformer = new SelectTransformer();
        $sql = 'SELECT * FROM users WHERE id = 1';
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice', 'email' => 'alice@example.com'], ['id' => 2, 'name' => 'Bob', 'email' => 'bob@example.com']], 'columns' => ['id', 'name', 'email'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INT'), 'name' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'VARCHAR(255)'), 'email' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'VARCHAR(255)')]]];
        $result = $transformer->transform($sql, $tables);
    ...
    public function testEmptyRowsWithColumnsReturnsWithClause(): void
    {
        $transformer = new SelectTransformer();
        $sql = 'SELECT * FROM users WHERE id = 1';
        $tables = ['users' => ['rows' => [], 'columns' => ['id', 'name', 'email'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INT'), 'name' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'VARCHAR(255)'), 'email' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'VARCHAR(255)')]]];
        $result = $transformer->transform($sql, $tables);
        self::assertStringStartsWith('WITH', ltrim($result));
        self::assertNotEmpty($result);
    }
