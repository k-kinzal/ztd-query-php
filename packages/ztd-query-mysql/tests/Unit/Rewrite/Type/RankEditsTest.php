...
namespace Tests\Unit\Rewrite\Type;

use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\TestCase;
use ZtdQuery\Platform\MySql\Rewrite\Type\RankEdits;
...
final class RankEditsTest extends TestCase
{
    ...
    {
        $left = new \ZtdQuery\Schema\ColumnDeclaration(\ZtdQuery\Schema\ColumnTypeFamily::STRING, "ENUM('new','done')");
        $right = new \ZtdQuery\Schema\ColumnDeclaration(\ZtdQuery\Schema\ColumnTypeFamily::STRING, "ENUM('off','on')");
        self::assertSame([
    ...
    public function testOrderByEditsLeavesArithmeticExpressionsUnchanged(): void
    {
        ...
        $edits = (new RankEdits())->orderByEdits($sql, \ZtdQuery\Sql\SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\MySql\Sql\MySqlLexerProfile::create())->significantTokens(), [], ['status' => ['new', 'done']]);
        ...
        self::assertSame([], (new RankEdits())->orderByEdits($sql, \ZtdQuery\Sql\SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\MySql\Sql\MySqlLexerProfile::create())->significantTokens(), [], ['status' => ['new', 'done']]));
    }
    ...
    {
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize('>= 1', \ZtdQuery\Platform\MySql\Sql\MySqlLexerProfile::create())->significantTokens();
        $edits = new RankEdits();
    ...
    {
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize('status', \ZtdQuery\Platform\MySql\Sql\MySqlLexerProfile::create())->significantTokens();
        $edits = [];
    ...
    {
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize('LIMIT `LIMIT`', \ZtdQuery\Platform\MySql\Sql\MySqlLexerProfile::create())->significantTokens();
        $edits = new RankEdits();
    ...
    {
        $tokens = \ZtdQuery\Sql\SqlTokenStream::tokenize('ASC , LIMIT score', \ZtdQuery\Platform\MySql\Sql\MySqlLexerProfile::create())->significantTokens();
        $edits = new RankEdits();
