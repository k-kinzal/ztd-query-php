...
use ZtdQuery\Platform\MySql\Sql\Value\ScalarExpression;
...
final class ScalarExpressionTest extends TestCase
{
    public function testRenderExpressionPreservesNumbersAndEscapesTextAndBinary(): void
    {
        ...
        $integer = new \ZtdQuery\Schema\ColumnDeclaration(\ZtdQuery\Schema\ColumnTypeFamily::INTEGER, 'INT');
        $text = new \ZtdQuery\Schema\ColumnDeclaration(\ZtdQuery\Schema\ColumnTypeFamily::STRING, 'VARCHAR');
        $binary = new \ZtdQuery\Schema\ColumnDeclaration(\ZtdQuery\Schema\ColumnTypeFamily::BINARY, 'BLOB');
