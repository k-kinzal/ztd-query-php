...
namespace ZtdQuery\Platform\MySql\Sql\Relation;
...
final class MySqlSelectRelationParser
{
    ...
    public function fromClauses(string $sql): array
    {
        ...
        foreach ($tokens as $token) {
            ...
            $end = (new ReferenceReader())->findFromEnd($sql, $tokens, $token);
    ...
    public function references(string $sql): array
    {
        ...
        foreach ($tokens as $token) {
            ...
            $clauseEnd = (new ReferenceReader())->findFromEnd($sql, $tokens, $token);
            $clause = substr($sql, $clauseStart, $clauseEnd - $clauseStart);
            foreach ((new ReferenceReader())->referencesFromClause($clause) as $reference) {
                $references[] = [
                    'name' => $reference['name'],
