...
namespace ZtdQuery\Platform\MySql\Sql\Transaction;
...
final class MySqlTransactionStatementParser implements TransactionStatementParser
{
    ...
    public function parse(string $sql): ?TransactionStatement
    {
        ...
        if ((new TokenReader())->matchesAny($tokens, [['BEGIN'], ['BEGIN', 'WORK'], ['START', 'TRANSACTION']])) {
            return TransactionStatement::begin();
        }
        if ((new TokenReader())->matchesAny($tokens, [['COMMIT'], ['COMMIT', 'WORK']])) {
            return TransactionStatement::commit();
        }
        if ((new TokenReader())->matchesAny($tokens, [['ROLLBACK'], ['ROLLBACK', 'WORK']])) {
            return TransactionStatement::rollback();
        }
        $name = (new TokenReader())->nameAfter($tokens, [['SAVEPOINT']]);
        if ($name !== null) {
            return TransactionStatement::savepoint($name);
        }
        $name = (new TokenReader())->nameAfter($tokens, [['ROLLBACK', 'TO'], ['ROLLBACK', 'TO', 'SAVEPOINT']]);
        if ($name !== null) {
            return TransactionStatement::rollbackTo($name);
        }
        $name = (new TokenReader())->nameAfter($tokens, [['RELEASE', 'SAVEPOINT']]);
