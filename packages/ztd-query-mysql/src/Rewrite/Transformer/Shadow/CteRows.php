...
namespace ZtdQuery\Platform\MySql\Rewrite\Transformer\Shadow;

use RuntimeException;
use ZtdQuery\Platform\CastRenderer;
use ZtdQuery\Platform\IdentifierQuoter;
use ZtdQuery\Platform\MySql\Rewrite\GeneratedColumn\MySqlGeneratedColumnProjector;
...
final class CteRows
{
    ...
    public function formatValue(mixed $val, ?ColumnDeclaration $type = null): string
    {
        if ($type !== null
            ...
            $val = (new \ZtdQuery\Platform\MySql\Rewrite\Transformer\Set\ValueNormalizer())->normalizeSetValue((string) $val, $type->nativeType);
        }
    ...
    public function emptySelect(array $columns, array $columnTypes): string
    {
        ...
        foreach ($columns as $column) {
            $type = $columnTypes[$column] ?? new ColumnDeclaration(ColumnTypeFamily::STRING, 'VARCHAR');
