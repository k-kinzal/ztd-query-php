...
namespace ZtdQuery\Platform\MySql\Rewrite\Cte;
...
final class MySqlCteShadowComposer
{
    ...
    public function compose(string $sql, array $tableCtes): string
    {
        ...
        foreach (array_reverse($tableCtes, true) as $table => $cte) {
            ...
            foreach ($requiredSql as $requiredPart) {
                if ((new \ZtdQuery\Platform\MySql\Sql\Cte\IdentifierReferences($this->lexerProfile))->referencesIdentifier($requiredPart, $table)) {
                    $referenced = true;
                }
            }
            if (!$referenced) {
    ...
    {
        return (new \ZtdQuery\Platform\MySql\Sql\Cte\HeaderParser($this->lexerProfile))->parseHeader($sql)['names'];
    }
    ...
    {
        $header = (new \ZtdQuery\Platform\MySql\Sql\Cte\HeaderParser($this->lexerProfile))->parseHeader($originalSql);
        ...
        if ($rewrittenWith !== null && $rewrittenWith->isKeyword('WITH')) {
            $rewrittenHeader = (new \ZtdQuery\Platform\MySql\Sql\Cte\HeaderParser($this->lexerProfile))->parseHeader($rewrittenStatement);
            $rewrittenStatementOffset = $rewrittenHeader['statementOffset'];
            if ($rewrittenStatementOffset === null) {
                return $prefix . "\n" . $rewrittenStatement;
            }

            $headerParser = new \ZtdQuery\Platform\MySql\Sql\Cte\HeaderParser($this->lexerProfile);
            ...
            if ((new \ZtdQuery\Platform\MySql\Sql\Cte\IdentifierReferences($this->lexerProfile))->referencesAnyIdentifier($rewrittenBody, $header['names'])) {
                return $prefix . ",\n" . $rewrittenBody . "\n" . $rewrittenTail;
            }
    ...
    {
        $offset = (new \ZtdQuery\Platform\MySql\Sql\Cte\HeaderParser($this->lexerProfile))->parseHeader($sql)['statementOffset'];
