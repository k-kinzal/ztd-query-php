 ...
 */
final class MySqlUpsertExpressionCursor
{
     ...
     * @param string|null $incomingAlias Name the statement gave the incoming row, or null where it gave none
     * @param list<SqlToken> $tokens The expression, with nothing insignificant left in
     * @param MySqlUpsertLiteral $literals Reads a literal or a name back out of how it was written
     */
    public function __construct(
        ...
        private readonly MySqlUpsertLiteral $literals = new MySqlUpsertLiteral(),
    ) {
    }
    ...
    public static function over(string $sql, string $tableName, ?string $incomingAlias = null): self
    {
        return new self(
            ...
            $tableName,
            $incomingAlias,
            SqlTokenStream::tokenize($sql, MySqlLexerProfile::create())->significantTokens(),
        );
    }
    ...
    {
        if ($this->incomingAlias !== null && strcasecmp($qualifier, $this->incomingAlias) === 0) {
