...
/**
 * Reads the parts of a SQLite CREATE TABLE statement.
 *
 ...
 * DECIMAL precision puts commas inside parentheses that are part of one
...
final class SqliteCreateTable
{
     ...
     *
     * A schema-qualified name is reduced to the table's own name, which is
     * what a fixture asks for it by.
     *
    ...
    {
        return preg_match('/^(PRIMARY\s+KEY|FOREIGN\s+KEY|UNIQUE|CHECK|CONSTRAINT)\b/i', trim($definition)) === 1;
    }

    /**
     * Answers the columns a table-level PRIMARY KEY names.
     *
     * A key declared beside its column is found while that column is read; one
     * declared on its own line is the only way a composite key can be written,
     * and it has to be read before the columns so that each of them knows it
     * is part of the key.
     *
     * @param string $columnsBlock Text between the outermost parentheses
     *
