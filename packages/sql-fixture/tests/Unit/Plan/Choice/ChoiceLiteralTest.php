...
use SqlFixture\Plan\Choice\ChoiceLiteral as Subject;
...
final class ChoiceLiteralTest extends TestCase
{
    /**
    ...
    {
        return [["'a,b;{c}''d'", "a,b;{c}'d"], ["'1'", '1'], ['1', 1], ['-12', -12], ['0', 0], ['true', true], ['false', false], ['null', null], ["''", '']];
    }

    #[DataProvider('providerLiterals')]
    public function testReadPreservesTypeAndAdvancesTheCursor(string $literal, string|int|bool|null $expected): void
    {
        $cursor = new RelationCursor($literal . ' {}');
        self::assertSame($expected, (new Subject())->read($cursor));
        self::assertSame(strlen($literal), $cursor->offset);
    }

    #[DataProvider('providerLiterals')]
    public function testPrintRoundTripsTypedValues(string $literal, string|int|bool|null $value): void
    {
        self::assertSame($literal, (new Subject())->print($value));
    }

    public function testIntegerAcceptsNegativeValues(): void
    {
        self::assertSame(-23, (new Subject())->integer('-23', new RelationCursor('-23 {}')));
    }


    #[DataProvider('providerLiterals')]
    public function testReadStartsAtTheSuppliedOffset(string $literal, string|int|bool|null $expected): void
    {
        $cursor = new RelationCursor('prefix ' . $literal . ' {}', 7);
        self::assertSame($expected, (new Subject())->read($cursor));
        self::assertSame(7 + strlen($literal), $cursor->offset);
    }
}
