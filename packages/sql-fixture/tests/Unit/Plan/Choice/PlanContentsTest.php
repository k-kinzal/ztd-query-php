...
use SqlFixture\Plan\Choice\PlanContents as Subject;
...
final class PlanContentsTest extends TestCase
{
    public function testKeepsTheSubjectAndCollectsEveryCandidateTable(): void
    {
        $post = Relation::manyToOne('comments.target_id', 'posts.id');
        $user = Relation::manyToOne('comments.user_id', 'users.id');
        $choice = RelationChoice::on('comments.kind')->when('post', $post)->when('none');
        $parts = new Subject(['posts', $choice, $user, 'audit']);
        self::assertSame(['posts', 'comments', 'users', 'audit'], $parts->tables);
        self::assertSame([$post, $user], $parts->relations);
        self::assertSame([$user], $parts->unconditional);
        self::assertSame([$choice], $parts->choices);
    }

    public function testAppendKeepsEndpointOrder(): void
    {
        $contents = new Subject([]);
        $relation = Relation::oneToMany('posts.id', 'comments.target_id');
        $contents->append($relation);
        self::assertSame(['posts', 'comments'], $contents->tables);
        self::assertSame([$relation], $contents->relations);
    }
