...
use SqlFixture\Plan\Choice\ChoiceValidation as Subject;
...
#[UsesClass(Subject::class)]
final class ChoiceValidationTest extends TestCase
{
    public function testValidateAllowsExclusiveBindings(): void
    {
        $choice = RelationChoice::on('c.kind')->when('a', Relation::manyToOne('c.id', 'a.id'))->when('b', Relation::manyToOne('c.id', 'b.id'));
        $contents = new \SqlFixture\Plan\Choice\PlanContents([$choice]);
        (new Subject())->validate($contents);
        self::assertSame([$choice], $contents->choices);
    }


    public function testValidateChoiceAllowsMultipleRelationsWithinACase(): void
    {
        $choice = RelationChoice::on('c.kind')->when('a', Relation::manyToOne('c.a_id', 'a.id'), Relation::manyToOne('c.b_id', 'b.id'));
        (new Subject())->validateChoice($choice, []);
        self::assertCount(2, $choice->cases[0]->relations);
    }

    public function testValidateRelationAcceptsIndependentBindings(): void
    {
        $choice = RelationChoice::on('c.kind');
        $relation = Relation::manyToOne('c.a_id', 'a.id');
        (new Subject())->validateRelation($choice, $relation, [Relation::manyToOne('c.b_id', 'b.id'), Relation::manyToOne('other.a_id', 'a.id')]);
        self::assertSame(['a_id' => 'id'], $relation->columnMap());
    }
}
