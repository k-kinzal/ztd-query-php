...
use SqlFixture\Fixture\Generation\RowBindings as Subject;
...
final class RowBindingsTest extends TestCase
{
    public function testMergePreservesInheritedValuesAndAddsOverrides(): void
    {
        self::assertSame(['id' => 7, 'kind' => 'post'], (new Subject())->merge('comments', ['id' => 7], ['id' => 7, 'kind' => 'post']));
        self::assertSame(['id' => null], (new Subject())->merge('comments', ['id' => null], ['id' => null]));
    }

    public function testMergeRejectsAConflictingReferenceIncludingNull(): void
    {
        $this->expectException(\SqlFixture\Fixture\Generation\RelationValueException::class);
        (new Subject())->merge('comments', ['target_id' => 7], ['target_id' => null]);
    }
