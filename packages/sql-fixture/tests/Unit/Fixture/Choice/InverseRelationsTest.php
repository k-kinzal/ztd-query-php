...
use SqlFixture\Fixture\Choice\InverseRelations as Subject;
...
final class InverseRelationsTest extends TestCase
{
    public function testUniqueVisitsEquivalentInverseCasesOnce(): void
    {
        $a = Relation::manyToOne('c.id', 'a.id');
        $b = Relation::manyToOne('c.id', 'b.id');
        $choice = RelationChoice::on('c.kind')->when('one', $a)->when('two', Relation::manyToOne('c.id', 'a.id'))->otherwise($b);
        $plain = Relation::manyToOne('c.user_id', 'users.id');
        $plan = new FixturePlan($choice, $plain);
        self::assertSame([$a, $b, $plain], (new Subject())->unique($plan, $plan->relations));
    }

    public function testKeyLeavesUnconditionalRelationsIndependent(): void
    {
        $relation = Relation::manyToOne('c.id', 'a.id');
        self::assertNull((new Subject())->key(new FixturePlan($relation), $relation));
        $plan = new FixturePlan(RelationChoice::on('c.kind')->when('a', $relation));
        self::assertSame('0:a.id:c.id:>', (new Subject())->key($plan, $relation));
    }
