...
final class SqliteCteShadowComposer
{
    ...
    {
        $ctes = (new Rewriting\Cte\CteDependencies())->required($sql, $tableCtes);
    ...
    {
        return (new Rewriting\Cte\CteHeaderParser())->parseHeader($sql)['names'];
    }
    ...
    {
        $header = (new Rewriting\Cte\CteHeaderParser())->parseHeader($originalSql);
        ...
        if ($rewrittenWith !== null && $rewrittenWith->isKeyword('WITH')) {
            $rewrittenHeader = (new Rewriting\Cte\CteHeaderParser())->parseHeader($rewrittenStatement);
            ...
            if ((new Rewriting\Cte\CteReferences())->referencesAnyIdentifier($rewrittenBody, $header['names'])) {
                return $prefix . ",\n" . $rewrittenBody . "\n" . $rewrittenTail;
            }

            return (new Rewriting\Cte\CtePrefixMerger())->prepend(
                $originalSql,
    ...
    {
        $offset = (new Rewriting\Cte\CteHeaderParser())->parseHeader($sql)['statementOffset'];
