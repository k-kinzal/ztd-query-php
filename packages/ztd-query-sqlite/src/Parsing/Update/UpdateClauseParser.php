...
namespace ZtdQuery\Platform\Sqlite\Parsing\Update;
...
final class UpdateClauseParser
{
    ...
    {
        $tokens = SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Sqlite\SqliteLexerProfile::create())->significantTokens();
        foreach ($tokens as $index => $token) {
            ...
            $index = (new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\IdentifierDecoder())->identifierEndIndex($tokens, $index);
            if (($tokens[$index] ?? null)?->kind === SqlTokenKind::Symbol
                && $tokens[$index]->text === '.'
            ) {
                $index = (new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\IdentifierDecoder())->identifierEndIndex($tokens, $index + 1);
            }
            ...
            return (new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\IdentifierDecoder())->unquoteIdentifier($alias->text);
        }
    ...
    {
        return SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Sqlite\SqliteLexerProfile::create())->topLevelClause(
            ['FROM'],
    ...
    {
        return SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Sqlite\SqliteLexerProfile::create())->topLevelClause(['LIMIT'], [['RETURNING']]);
    }
    ...
    {
        return SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Sqlite\SqliteLexerProfile::create())->topLevelClauseAfter(
            ['DO', 'UPDATE', 'SET'],
            ['WHERE'],
            [['RETURNING']],
        );
