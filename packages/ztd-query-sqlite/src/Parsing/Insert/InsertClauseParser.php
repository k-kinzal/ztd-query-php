...
namespace ZtdQuery\Platform\Sqlite\Parsing\Insert;
...
final class InsertClauseParser
{
    ...
    {
        $sql = (new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\LiteralMasker())->stripComments($sql);
        if (preg_match('/\bINTO\s+(?:"(?:[^"]|"")*"|[^\s(]+)\s*\(([^)]+)\)\s*(?:VALUES|SELECT)/i', $sql, $matches) === 1) {
            return (new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\IdentifierDecoder())->parseColumnList($matches[1]);
        }
    ...
    public function extractInsertValues(string $sql): array
    {
        ...
        return (new \ZtdQuery\Platform\Sqlite\Parsing\Expression\ValueListParser())->parseValueSets($rest);
    }
    ...
    {
        $trimmed = ltrim((new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\LiteralMasker())->stripComments($sql));
    ...
    {
        $foundInsert = false;
        foreach ((new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\TopLevelKeywordScanner())->scanTopLevelKeywords($sql) as $token) {
            if (!$foundInsert) {
