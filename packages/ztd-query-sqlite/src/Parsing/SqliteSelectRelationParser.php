...
final class SqliteSelectRelationParser
{
    ...
    {
        $tokens = (new Parsing\Relation\RelationSourceParser())->tokens($sql);
        /**
        ...
        foreach ($tokens as $token) {
            ...
            $end = (new Parsing\Relation\RelationSourceParser())->findFromEnd($sql, $tokens, $token);
    ...
    public function references(string $sql): array
    {
        ...
        foreach ($tokens as $token) {
            ...
            $clauseEnd = (new Parsing\Relation\RelationSourceParser())->findFromEnd($sql, $tokens, $token);
            $clause = substr($sql, $clauseStart, $clauseEnd - $clauseStart);
            foreach ((new Parsing\Relation\RelationSourceParser())->referencesFromClause($clause) as $reference) {
                $references[] = [
                    'name' => $reference['name'],
