...
final class AssignmentColumnParser
{
    ...
    public function parse(string $setClause, int $i): array
    {
        ...
        $colName = (new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\IdentifierDecoder())->unquoteIdentifier(trim(substr($setClause, $colStart, $i - $colStart)));
        if (str_contains($colName, '.')) {
            $parts = explode('.', $colName);
            $colName = (new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\IdentifierDecoder())->unquoteIdentifier(trim(end($parts)));
        }
