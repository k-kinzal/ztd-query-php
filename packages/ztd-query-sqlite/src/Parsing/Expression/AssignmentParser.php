...
namespace ZtdQuery\Platform\Sqlite\Parsing\Expression;
...
final class AssignmentParser
{
    ...
    {
        $setClause = SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Sqlite\SqliteLexerProfile::create())->topLevelClause(
            ['SET'],
    ...
    {
        $action = SqlTokenStream::tokenize($sql, \ZtdQuery\Platform\Sqlite\SqliteLexerProfile::create())->topLevelClause(['DO'], [['RETURNING']]);
        if ($action === null) {
            return [];
        }
        $actionStream = SqlTokenStream::tokenize($action, \ZtdQuery\Platform\Sqlite\SqliteLexerProfile::create());
    ...
    public function parseAssignments(string $setClause): array
    {
        ...
        while ($index < $length) {
            ...
            $end = (new \ZtdQuery\Platform\Sqlite\Parsing\Lexing\ExpressionSpan())->end($setClause, $index);
