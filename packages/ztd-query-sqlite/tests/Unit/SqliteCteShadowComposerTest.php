...
final class SqliteCteShadowComposerTest extends TestCase
{
    ...
    public function testParseHeaderReadsWhatTheWithNamesAndWhereTheStatementStarts(): void
    {
        self::assertSame(
            ...
            (new SqliteCteShadowComposer())->parseHeader('WITH x AS (SELECT 1) SELECT * FROM x'),
        );
    }
    ...
    public function testParseHeaderIsEmptyForAStatementThatOpensWithNoWith(): void
    {
        self::assertSame(
            ...
            (new SqliteCteShadowComposer())->parseHeader('SELECT 1'),
        );
    }

    public function testFindAsIndexAnswersWhereTheAsIsWritten(): void
    {
        $tokens = SqlTokenStream::tokenize('x AS (SELECT 1)', SqliteLexerProfile::create())->significantTokens();

        self::assertSame(1, (new SqliteCteShadowComposer())->findAsIndex($tokens, 1));
    }

    public function testFindAsIndexIsNothingWhereABareWordComesFirst(): void
    {
        $tokens = SqlTokenStream::tokenize('x y', SqliteLexerProfile::create())->significantTokens();

        self::assertNull((new SqliteCteShadowComposer())->findAsIndex($tokens, 1));
    }
    ...
    public function testIsSymbolReportsATokenBeingThatSymbol(): void
    {
        ...
        self::assertTrue((new SqliteCteShadowComposer())->isSymbol($tokens[0], '('));
    }

    public function testIsSymbolIsFalsePastTheEndOfWhatWasWritten(): void
    {
        self::assertFalse((new SqliteCteShadowComposer())->isSymbol(null, '('));
    }

    public function testReferencesIdentifierReportsAStatementNamingIt(): void
    {
        self::assertTrue((new SqliteCteShadowComposer())->referencesIdentifier('SELECT * FROM users', 'users'));
    }
    ...
    {
        self::assertFalse((new SqliteCteShadowComposer())->referencesIdentifier('SELECT * FROM orders', 'users'));
    }

    public function testReferencesAnyIdentifierReportsAStatementNamingOneOfThem(): void
    {
        self::assertTrue(
            (new SqliteCteShadowComposer())->referencesAnyIdentifier('SELECT * FROM users', ['orders', 'users']),
        );
    }
    ...
    {
        self::assertFalse(
            (new SqliteCteShadowComposer())->referencesAnyIdentifier('SELECT 1', ['orders', 'users']),
        );
    }
    ...
    public function testIdentifierNameTakesTheQuotingOffAName(): void
    {
        ...
        self::assertSame('order', (new SqliteCteShadowComposer())->identifierName($tokens[0]));
    }
    ...
    public function testIdentifierNameIsNothingForATokenThatIsNotAName(): void
    {
        ...
        self::assertNull((new SqliteCteShadowComposer())->identifierName($tokens[0]));
    }
    ...
    public function testDeclaredCteNamesAnswersWhatTheStatementDeclaresForItself(): void
    {
        self::assertSame(
            ['x'],
            (new SqliteCteShadowComposer())->declaredCteNames('WITH x AS (SELECT 1) SELECT * FROM x'),
        );
    }
    ...
    public function testStatementSqlAnswersTheStatementWithoutItsHeader(): void
    {
        self::assertSame(
            'SELECT * FROM x',
            (new SqliteCteShadowComposer())->statementSql('WITH x AS (SELECT 1) SELECT * FROM x'),
        );
    }
}
