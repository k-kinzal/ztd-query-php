<?php

declare(strict_types=1);

namespace SqlSemantics\Statement\Model\MySql\Value;

/**
 * The join_table SQL value; generated by bin/build-models.php.
 *
 * @visibility public
 * @example Accept a structured SQL value
 *     $write = static fn (\SqlSemantics\Statement\Model\MySql\Value\JoinTableWithTableRefLeftOptOuterJoinSymTableFactorUsingUsingList_e05831d5 $value): string => (new \SqlSemantics\Statement\Statement($value))->toString();
 *     $write instanceof \Closure // => true
 */
final class JoinTableWithTableRefLeftOptOuterJoinSymTableFactorUsingUsingList_e05831d5 implements \SqlSemantics\Statement\Model\MySql\Role\DerivedTableListForm, \SqlSemantics\Statement\Model\MySql\Role\EscTableRefForm, \SqlSemantics\Statement\Model\MySql\Role\FromTablesForm, \SqlSemantics\Statement\Model\MySql\Role\JoinTableForm, \SqlSemantics\Statement\Model\MySql\Role\JoinTableListForm, \SqlSemantics\Statement\Model\MySql\Role\SelectDerivedForm, \SqlSemantics\Statement\Model\MySql\Role\TableRefForm, \SqlSemantics\Statement\Model\MySql\Role\TableReferenceListForm
{
    /**
     * Supplies the SQL values of this form.
     */
    public function __construct(
        public readonly \SqlSemantics\Statement\Model\MySql\Role\TableRefForm $tableRef,
        public readonly \SqlSemantics\Statement\Model\MySql\Role\OptOuterForm $optOuter,
        public readonly \SqlSemantics\Statement\Model\MySql\Role\TableFactorForm $tableFactor,
        public readonly \SqlSemantics\Statement\Model\MySql\Role\UsingListForm $usingList,
    ) {
    }

    /**
     * Writes SQL entirely from this value's fields.
     */
    public function write(\SqlSemantics\Statement\Writer $writer): void
    {
        $this->tableRef->write($writer);
        $writer->append('LEFT');
        $this->optOuter->write($writer);
        $writer->append('JOIN');
        $this->tableFactor->write($writer);
        $writer->append('USING');
        $writer->append('(');
        $this->usingList->write($writer);
        $writer->append(')');
    }
}
