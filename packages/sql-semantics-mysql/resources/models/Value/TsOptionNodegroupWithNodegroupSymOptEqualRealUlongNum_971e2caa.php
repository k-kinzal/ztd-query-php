<?php

declare(strict_types=1);

namespace SqlSemantics\Statement\Model\MySql\Value;

/**
 * The ts_option_nodegroup SQL value; generated by bin/build-models.php.
 *
 * @visibility public
 * @example Accept a structured SQL value
 *     $write = static fn (\SqlSemantics\Statement\Model\MySql\Value\TsOptionNodegroupWithNodegroupSymOptEqualRealUlongNum_971e2caa $value): string => (new \SqlSemantics\Statement\Statement($value))->toString();
 *     $write instanceof \Closure // => true
 */
final class TsOptionNodegroupWithNodegroupSymOptEqualRealUlongNum_971e2caa implements \SqlSemantics\Statement\Model\MySql\Role\LogfileGroupOptionForm, \SqlSemantics\Statement\Model\MySql\Role\LogfileGroupOptionListForm, \SqlSemantics\Statement\Model\MySql\Role\LogfileGroupOptionsForm, \SqlSemantics\Statement\Model\MySql\Role\OptLogfileGroupOptionsForm, \SqlSemantics\Statement\Model\MySql\Role\OptTablespaceOptionsForm, \SqlSemantics\Statement\Model\MySql\Role\TablespaceOptionForm, \SqlSemantics\Statement\Model\MySql\Role\TablespaceOptionListForm, \SqlSemantics\Statement\Model\MySql\Role\TablespaceOptionsForm, \SqlSemantics\Statement\Model\MySql\Role\TsOptionNodegroupForm
{
    /**
     * Supplies the SQL values of this form.
     */
    public function __construct(
        public readonly \SqlSemantics\Statement\Model\MySql\Role\OptEqualForm $optEqual,
        public readonly \SqlSemantics\Statement\Model\MySql\Role\RealUlongNumForm $realUlongNum,
    ) {
    }

    /**
     * Writes SQL entirely from this value's fields.
     */
    public function write(\SqlSemantics\Statement\Writer $writer): void
    {
        $writer->append('NODEGROUP');
        $this->optEqual->write($writer);
        $this->realUlongNum->write($writer);
    }
}
