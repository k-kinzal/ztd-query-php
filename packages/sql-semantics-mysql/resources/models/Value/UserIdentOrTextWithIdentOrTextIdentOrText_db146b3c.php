<?php

declare(strict_types=1);

namespace SqlSemantics\Statement\Model\MySql\Value;

/**
 * The user_ident_or_text SQL value; generated by bin/build-models.php.
 *
 * @visibility public
 * @example Accept a structured SQL value
 *     $write = static fn (\SqlSemantics\Statement\Model\MySql\Value\UserIdentOrTextWithIdentOrTextIdentOrText_db146b3c $value): string => (new \SqlSemantics\Statement\Statement($value))->toString();
 *     $write instanceof \Closure // => true
 */
final class UserIdentOrTextWithIdentOrTextIdentOrText_db146b3c implements \SqlSemantics\Statement\Model\MySql\Role\GrantListForm, \SqlSemantics\Statement\Model\MySql\Role\GrantUserForm, \SqlSemantics\Statement\Model\MySql\Role\PrivilegeCheckDefForm, \SqlSemantics\Statement\Model\MySql\Role\UserForm, \SqlSemantics\Statement\Model\MySql\Role\UserIdentOrTextForm, \SqlSemantics\Statement\Model\MySql\Role\UserListForm
{
    /**
     * Supplies the SQL values of this form.
     */
    public function __construct(
        public readonly \SqlSemantics\Statement\Model\MySql\Role\IdentOrTextForm $identOrText,
        public readonly \SqlSemantics\Statement\Model\MySql\Role\IdentOrTextForm $identOrText2,
    ) {
    }

    /**
     * Writes SQL entirely from this value's fields.
     */
    public function write(\SqlSemantics\Statement\Writer $writer): void
    {
        $this->identOrText->write($writer);
        $writer->append('@');
        $this->identOrText2->write($writer);
    }
}
