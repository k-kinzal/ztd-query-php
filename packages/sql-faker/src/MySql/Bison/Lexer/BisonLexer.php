...
final class BisonLexer
{
    ...
    private function token(): BisonToken
    {
        ...
        return match (true) {
            ...
            str_contains('=,)([]', $ch) => $this->tokenMiscChar($offset),
    ...
    private function tokenMiscChar(int $offset): BisonToken
    {
        $ch = $this->input[$this->pos];
        $this->pos++;
        return new BisonToken(BisonTokenType::CharLiteral, $ch, $offset);
    }

    /**
     * Advance the lexer and return the token value as a string.
     */
    ...
    /**
     * Advance the lexer and return the token value as an int.
     */
    public function nextInt(): int
    {
        $val = $this->next()->value;

        return is_int($val) ? $val : (int) $val;
    }

    /**
     * Consume all remaining input as raw text (used for epilogue section).
     */
    public function consumeRemaining(): string
    {
        ...
        $remaining = substr($this->input, $this->pos);
        $this->pos = $this->length;
        return $remaining;
    }
}
