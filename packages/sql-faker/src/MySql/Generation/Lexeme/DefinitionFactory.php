...
final class DefinitionFactory
{
    ...
    public function lexemes(string $version): LexemeGenerator
    {
        ...
        return new ChoiceLexemeGenerator(
            $this->contextualValues($version),
            new KeywordLexemeGenerator($keywords['symbols'], $keywords['functions']),
            new VersionedLexemeGenerator($version, new VersionCase(
                ...
                new ChoiceLexemeGenerator(
                    ...
                    $this->values(),
                    $this->dollarStrings($version),
                    $this->symbols($version)
                ),
    ...
    {
        return new ReverseLexemeGenerator($this->lexemes($version), new CandidateResolver(
    ...
    /**
     * sql/sql_lex.cc: value-producing scanner states.
     */
    ...
    /**
     * MY_LEX_IDENT, MY_LEX_USER_VARIABLE_DELIMITER and MY_LEX_HOSTNAME.
    ...
    {
        return new ChoiceLexemeGenerator(
            new ValueLexemeGenerator('IDENT', new IdentifierDomain('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_$', 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_$', '_sf', 63), ['_sqlfaker_identifier'], 'identifier', 'sql/sql_lex.cc:MY_LEX_IDENT'),
            new ValueLexemeGenerator('IDENT_QUOTED', new QuotedDomain('`', minimum: 1, maximum: 64, trailingSpace: false), ['`name`'], 'quoted-identifier', 'sql/sql_lex.cc:MY_LEX_USER_VARIABLE_DELIMITER'),
            new ValueLexemeGenerator('LEX_HOSTNAME', new CharacterDomain(str_split('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_.$'), 1, 64), ['localhost'], 'hostname', 'sql/sql_lex.cc:MY_LEX_HOSTNAME'),
            new CharsetLexemeGenerator(),
        );
    }

    /**
     * get_text: single quotes, doubled delimiters and backslash escapes in the default SQL mode.
     * MY_LEX_IDENT_OR_NCHAR: N-prefixed national character literals.
     * @see https://github.com/mysql/mysql-server/blob/mysql-8.4.7/sql/sql_lex.cc
     */
    public function strings(): LexemeGenerator
    {
        $ascii = array_map(chr(...), range(1, 127));
        return new ChoiceLexemeGenerator(
            new CharsetValueLexemeGenerator(
                new ValueLexemeGenerator('TEXT_STRING', new QuotedDomain("'", backslash: true, alphabet: [...$ascii, 'é', '猫']), ["'text'", "'a''b'"], 'string', 'sql/sql_lex.cc:get_text'),
                new ValueLexemeGenerator('TEXT_STRING', new QuotedDomain("'", backslash: true, alphabet: $ascii), ["'text'", "'a''b'"], 'string', 'sql/sql_lex.cc:get_text'),
            ),
            new ValueLexemeGenerator('NCHAR_STRING', new QuotedDomain("'", ['N', 'n'], true, alphabet: [...$ascii, 'é', '猫']), ["N'text'"], 'string', 'sql/sql_lex.cc:MY_LEX_IDENT_OR_NCHAR'),
        );
    }

    /**
     * int_token boundaries and MY_LEX_REAL decimal / exponent branches.
     * @see https://github.com/mysql/mysql-server/blob/mysql-8.4.7/sql/sql_lex.cc
     */
    public function numbers(): LexemeGenerator
    {
        $digits = new CharacterDomain(str_split('0123456789'), 1, 20);
        $decimal = new ChoiceDomain(
            new SequenceDomain($digits, new WordDomain(['.']), new CharacterDomain(str_split('0123456789'), 0, 30)),
            new SequenceDomain(new WordDomain(['.']), $digits),
        );
        ...
        return new ChoiceLexemeGenerator(
            ...
            new ValueLexemeGenerator('DECIMAL_NUM', $decimal, ['1.5'], 'number', 'sql/sql_lex.cc:MY_LEX_REAL'),
            new ValueLexemeGenerator('FLOAT_NUM', new SequenceDomain(new ChoiceDomain($digits, $decimal), $exponent), ['1e2'], 'number', 'sql/sql_lex.cc:MY_LEX_REAL:exponent'),
        );
    }

    /**
     * Character introducers constrain sampled bytes before lexeme construction.
     * @see https://github.com/mysql/mysql-server/blob/mysql-8.4.7/sql/sql_lex.cc
     */
    ...
    /**
     * MY_LEX_HEX_NUMBER and MY_LEX_BIN_NUMBER; quoted hex contains whole bytes.
     * @see https://github.com/mysql/mysql-server/blob/mysql-8.4.7/sql/sql_lex.cc
     */
    public function binaryDomain(bool $character): LexemeGenerator
    {
        return new ChoiceLexemeGenerator(
            new ValueLexemeGenerator('HEX_NUM', new RadixDomain('0123456789abcdefABCDEF', '0x', ['X', 'x'], 2, 32, $character), [sprintf('0x%02x', 15), "X'0f'"], 'number', 'sql/sql_lex.cc:MY_LEX_HEX_NUMBER'),
            new ValueLexemeGenerator('BIN_NUM', new RadixDomain('01', '0b', ['B', 'b'], 1, 64, $character), ['0b01', "B'01'"], 'number', 'sql/sql_lex.cc:MY_LEX_BIN_NUMBER'),
        );
    }
     ...
     */
    public function contextualValues(string $version): LexemeGenerator
    {
        return new VersionedLexemeGenerator(
            $version,
            new VersionCase(['mysql-5.7.44'], new ChoiceLexemeGenerator($this->contextualWord('ROTATE_KEY_ENGINE', ['INNODB'], 'alter_instance_action'), new ReplicationTablePatternLexemeGenerator()), 'mysql-5.7-key-rotation'),
            new VersionCase(
                ...
                new ChoiceLexemeGenerator(
                    ...
                    new ValueLexemeGenerator('EXTERNAL_ROUTINE_LANGUAGE', new IdentifierDomain('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_', 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_', excluded: ['SQL']), ['javascript'], 'identifier', 'sql/sql_yacc.yy:stored_routine_body'),
                    new BoundedIntegerLexemeGenerator('BINLOG_RESET_INDEX', 1, 2000000000, ['1', '2000000000', "X'01'"], 'sql/sql_yacc.yy:source_reset_options'),
                    $this->contextualWord('ROTATE_KEY_ENGINE', ['INNODB', 'BINLOG'], 'alter_instance_action'),
                    $this->contextualWord('REDO_ENGINE', ['INNODB'], 'alter_instance_action'),
                    $this->contextualWord('REDO_LOG_NAME', ['REDO_LOG'], 'alter_instance_action'),
                    $this->contextualWord('LOAD_COUNT_NAME', ['COUNT'], 'opt_source_count'),
                    new IntegerLexemeGenerator('LOAD_SOURCE_COUNT', '1', '2147483647', ['1'], 'sql/sql_yacc.yy:opt_source_count'),
                    new BoundedIntegerLexemeGenerator('REPLICATION_FLAG_NUMBER', 0, 1, ['0', '1'], 'sql/sql_yacc.yy:SOURCE_CONNECTION_AUTO_FAILOVER'),
                    new BoundedIntegerLexemeGenerator('TERNARY_OPTION_NUMBER', 0, 1, ['0', '1'], 'sql/sql_yacc.yy:ternary_option'),
                ),
    ...
    {
        return new ValueLexemeGenerator($terminal, new WordDomain($words, true), $words, 'identifier', 'sql/sql_yacc.yy:' . $rule);
    }

    /**
     * Declares scanner output not supplied by lex.h keyword registrations.
     */
    public function symbols(string $version): LexemeGenerator
    {
        $symbols = [];
     ...
     * @return non-empty-list<string>
     */
    public function dollarVersions(): array
    {
     ...
     */
    public function dollarStrings(string $version): LexemeGenerator
    {
        return new VersionedLexemeGenerator($version, new VersionCase(
            $this->dollarVersions(),
            new ValueLexemeGenerator('DOLLAR_QUOTED_STRING_SYM', new DollarQuotedDomain(
                new CharacterDomain([...str_split('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_'), 'é', '猫'], 0, 16),
                new CharacterDomain([...array_map(chr(...), [...range(1, 35), ...range(37, 127)]), 'é', '猫'], 0, 255),
            ), ['$$text$$', '$tag$text$tag$'], 'string', 'sql/sql_lex.cc:get_dollar_quoted_text'),
            'mysql-dollar-quoted-strings',
        ));
    }

    /**
     * Exact lex.h spelling declarations, grouped only when all listed releases agree.
     * SYM and SYM_HK enter keyword lookup; SYM_FN additionally requires function context.
     * @throws RuntimeException When the exact release is unsupported
    ...
    public function keywords(string $version): array
    {
        ...
        $symbols = [];
        $functions = [];
        foreach (self::KEYWORD_CASES as [$versions, $keywords, $calls]) {
    ...
    /**
     * Fixed spellings next to their parser terminals, with exact source releases.
     * @var list<array{list<string>, array<string, list<string>>, array<string, list<string>>}>
     */
