...
namespace Tests\Unit\Generation\Candidate;
...
final class IntegerLexemeGeneratorTest extends TestCase
{
    ...
    {
        $generator = new IntegerLexemeGenerator('FCONST', '2147483648', null, ['2147483648'], 'scanner:overflow');
        $input = new LexemeInput(TerminalSequence::fromNames(['FCONST']), 0, new ResolvedOutput(), values: new \SqlFaker\Generation\Value\ValueChoices(static fn (int $count): int => $count - 1));
        $result = $generator->generate($input);
        self::assertNotNull($result);
