...
namespace Tests\Unit\PostgreSql\Generation\Rewrite;
...
final class WindowFrameRuleTest extends TestCase
{
    ...
    {
        $trace = new DerivationTrace('frame_extent');
        $trace->expand(0, new Production([new Terminal('BETWEEN'), new NonTerminal('frame_bound'), new Terminal('AND'), new NonTerminal('frame_bound')]), 0);
    ...
    public static function providerFrames(): iterable
    {
        ...
        yield 'forbidden start' => [['UNBOUNDED', 'FOLLOWING'], ['CURRENT', 'ROW'], ['BETWEEN', 'UNBOUNDED', 'PRECEDING', 'AND', 'CURRENT', 'ROW']];
        yield 'forbidden end' => [['UNBOUNDED', 'PRECEDING'], ['UNBOUNDED', 'PRECEDING'], ['BETWEEN', 'UNBOUNDED', 'PRECEDING', 'AND', 'UNBOUNDED', 'FOLLOWING']];
    }
