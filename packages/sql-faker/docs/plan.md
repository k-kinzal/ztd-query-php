...
The budget must lie between `minimumExpansions()` and the constraints' expansion budget, 5000 without one. `SqlFaker\Generation\Seed\SeedCorpusBuilder` uses the encoder to write one such input for every production a start rule can reach; the repository's `seeds/` directory holds the results for the default grammar versions.
## MySQL
