...
final class QueryRulesTest extends TestCase
{
    ...
    public function testJoinedTracksNullableInput(): void
    {
        $schema = (new SchemaBuilder(Dialect::PostgreSql))->build('CREATE TABLE items (id INTEGER PRIMARY KEY, label TEXT)');
        $bound = (new Binder($schema))->bind('SELECT b.id FROM items a LEFT JOIN items b ON a.id = b.id');
        self::assertSame(['j0'], $bound->outputs[0]->expression->nullExtendedBy);
    }
}
