...
final class SchemaRulesTest extends TestCase
{
    ...
    {
        $schema = (new SchemaBuilder(Dialect::Sqlite))->build('CREATE TABLE first (id INT)', 'CREATE TABLE second (id INT)');
        self::assertNotSame(Dialect::Sqlite->platform()->schema()->tableKey($schema->tables[0]), Dialect::Sqlite->platform()->schema()->tableKey($schema->tables[1]));
    }
