...
{
    public function testLegacyNamesKeepWorking(): void
    {
        $schema = (new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::Sqlite))->build('CREATE TABLE items (id INTEGER)');
        $bound = (new \SqlSemantics\Binder($schema))->bind('SELECT id FROM items');
        self::assertSame('id', $bound->outputs[0]->name);
    }
}
