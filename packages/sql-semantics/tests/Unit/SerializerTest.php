...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlSemantics\Serializer::class)]
...
{
    public function testSerializeAcceptsAReplaceableFormattingPolicy(): void
    {
        $serializer = new class () implements \SqlSemantics\Serializer {
            public function serialize(\SqlSemantics\Model\BoundStatement $statement): string
            {
                return (new \SqlSemantics\SimpleSerializer())->serialize($statement) . ';';
            }
        };
        $statement = (new Binder((new SchemaBuilder(Dialect::Sqlite))->build()))->bind('SELECT 1');
        self::assertInstanceOf(\SqlSemantics\Model\BoundSelect::class, $statement);
        self::assertSame('SELECT 1;', $serializer->serialize($statement));
    }
}
