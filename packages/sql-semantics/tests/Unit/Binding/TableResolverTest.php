...
final class TableResolverTest extends TestCase
{
    public function testResolvesQualifiedDeclarations(): void
    {
        ...
        $schema = $case->schema('CREATE TABLE app.users (id INTEGER)');
        $query = $case->analyzer->analyze($case->parser->parse('SELECT app.users.id FROM app.users'), $schema);
        self::assertSame('app', $query->relations[0]->declaration->schema);
        self::assertSame($schema->tables[0], $query->outputs[0]->expression->binding?->table);
    }
    public function testRejectsMissingTablesInsteadOfInventingColumns(): void
    {
        $this->expectException(SemanticException::class);
        (new AnalysisCase())->query('SELECT id FROM absent');
    }
