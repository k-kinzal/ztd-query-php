...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlSemantics\StatementFactory::class)]
...
final class StatementFactoryTest extends TestCase
{
    public function testSelectConstructsQuotedValuesAndResolvesReferencesWithoutSql(): void
    {
        $schema = (new SchemaBuilder(Dialect::PostgreSql))->build('CREATE TABLE t(id INTEGER)');
        $value = Expression::binary('+', Expression::reference(['id'], $schema->dialect), Expression::literal(1, $schema->dialect));
        $statement = (new \SqlSemantics\StatementFactory($schema))->select([new OutputColumn(0, 'next', $value)], $schema->tables[0]);
        self::assertSame('next', $statement->outputs[0]->name);
    ...
    public function testCreateValidatesCompleteConstructedStructure(): void
    {
        $schema = (new SchemaBuilder(Dialect::Sqlite))->build();
        $original = (new Binder($schema))->bind('BEGIN');
        ...
        self::assertSame('BEGIN', $statement->toString());
        self::assertNotSame($original, $statement);
    }
    public function testSelectCannotInjectAdditionalStatementsThroughLiteralOrAlias(): void
    {
        $schema = (new SchemaBuilder(Dialect::MySql))->build();
        $name = 'x`; DROP TABLE t; --';
        $statement = (new \SqlSemantics\StatementFactory($schema))->select([new OutputColumn(0, $name, Expression::literal("a'\\; DROP TABLE t; --", $schema->dialect))]);
        self::assertCount(1, $statement->outputs);
        self::assertSame($name, $statement->outputs[0]->name);
        self::assertSame([], $statement->diagnostics);
    }
    public function testCreateRevalidatesNamesAgainstTheDestinationSchema(): void
    {
        $sourceSchema = (new SchemaBuilder(Dialect::PostgreSql))->build('CREATE TABLE t(id INTEGER)');
        $statement = (new Binder($sourceSchema))->bind('SELECT id FROM t');
        self::assertInstanceOf(\SqlSemantics\Model\BoundSelect::class, $statement);
        $factory = new \SqlSemantics\StatementFactory((new SchemaBuilder(Dialect::PostgreSql))->build('CREATE TABLE t(value TEXT)'));
        $this->expectException(\SqlSemantics\SemanticException::class);
        $factory->create($statement);
    }
}
