...
final class StatementFactoryTest extends TestCase
{
    ...
    {
        $factory = new \SqlSemantics\StatementFactory((new SchemaBuilder(Dialect::Sqlite))->build());
        $statement = $factory->create(new Sql\Tree('command', [new Sql\Atom('keyword', 'BEGIN')]));
    ...
    public function testCreateRejectsMultipleStatements(): void
    {
        $factory = new \SqlSemantics\StatementFactory((new SchemaBuilder(Dialect::PostgreSql))->build());
        $this->expectException(\SqlSemantics\SemanticException::class);
        $factory->create(new Sql\Tree('invalid', [new Sql\Atom('keyword', 'SELECT 1; SELECT 2')]));
    }
}
