...
final class NamespaceBindingTest extends TestCase
{
    public function testDistinguishesPrefixedFromDefaultNamespaces(): void
    {
        ...
        self::assertSame('SELECT "x"."n" AS "n" FROM XMLTABLE(XMLNAMESPACES(\'urn:x\' AS "x", DEFAULT \'urn:d\'), \'/x:rows/x:row\' PASSING \'<rows/>\' COLUMNS "n" FOR ORDINALITY) AS "x"', $statement->toString());
