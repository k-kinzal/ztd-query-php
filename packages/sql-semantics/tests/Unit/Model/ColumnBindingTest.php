...
#[Medium]
final class ColumnBindingTest extends TestCase
{
    public function testRetainsDeclarationIdentity(): void
    {
        $query = (new AnalysisCase())->query('SELECT a.id FROM users a');
        self::assertSame($query->relations[0]->declaration->columns[0], $query->outputs[0]->expression->binding?->column);
    }
