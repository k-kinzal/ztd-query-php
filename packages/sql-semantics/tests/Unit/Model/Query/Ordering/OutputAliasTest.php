...
final class OutputAliasTest extends TestCase
{
    public function testRefersToTheOutputWithoutRepeatingItsComputation(): void
    {
        ...
        self::assertSame(1, substr_count(strtoupper((new \SqlSemantics\SimpleSerializer())->serialize($statement)), '"RANDOM"()'));
        self::assertStringEndsWith('ORDER BY "draw" ASC', (new \SqlSemantics\SimpleSerializer())->serialize($statement));
    }
    ...
    public function testWindowOrderingUsesItsInputExpression(): void
    {
        ...
        self::assertInstanceOf(\SqlSemantics\Model\BoundSelect::class, $statement);
        self::assertSame('SELECT "count"(*) OVER (ORDER BY 1 ASC) AS "n"', (new \SqlSemantics\SimpleSerializer())->serialize($statement));
    }
}
