...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlSemantics\Model\Transformation\SourceEdit::class)]
...
final class SourceEditTest extends TestCase
{
    public function testReplaceMatchesSyntheticComponentsByOwnedTerminals(): void
    {
        $a = new \SqlParser\Lexer\Token(1, 'number', '1', 12);
        $b = new \SqlParser\Lexer\Token(1, 'number', '2', 20);
        $source = new \SqlParser\Parser\Node('values', 0, [$a,$b]);
        $synthetic = new \SqlParser\Parser\Node('value', 0, [$b]);
        $changed = \SqlSemantics\Model\Transformation\SourceEdit::replace($source, Sql\Source::read($source), $synthetic, Expression::literal(3, Dialect::PostgreSql)->sql);
        self::assertSame('1 3', $changed->toString());
        self::assertSame('1 2', Sql\Source::read($source)->toString());
    }
    public function testReplaceRejectsForeignTokensWithTheSameSpelling(): void
    {
        $source = new \SqlParser\Lexer\Token(1, 'number', '1', 0);
        $foreign = new \SqlParser\Lexer\Token(1, 'number', '1', 0);
        $this->expectException(InvalidStructure::class);
        \SqlSemantics\Model\Transformation\SourceEdit::replace($source, Sql\Source::read($source), $foreign, Expression::literal(2, Dialect::PostgreSql)->sql);
    }
    public function testProductionCanReplaceAnEmptyOptionalPosition(): void
    {
        $empty = new \SqlParser\Parser\Node('optional', 0, []);
        $source = new \SqlParser\Parser\Node('query', 0, [$empty]);
        $replacement = Expression::literal(1, Dialect::PostgreSql)->sql;
        $changed = \SqlSemantics\Model\Transformation\SourceEdit::production($source, Sql\Source::read($source), $empty, $replacement);
