...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlSemantics\Model\Transformation\TreeEdit::class)]
...
final class TreeEditTest extends TestCase
{
    public function testReplaceRetainsOriginalTreeAndUsesIdentity(): void
    {
        $leaf = new Sql\Tree('number', [new Sql\Atom('number', '1')]);
        $tree = new Sql\Tree('statement', [$leaf]);
        $changed = \SqlSemantics\Model\Transformation\TreeEdit::replace($tree, $leaf, Expression::literal(2, Dialect::PostgreSql)->sql);
        self::assertSame('2', $changed->toString());
        self::assertSame('1', $tree->toString());
    }
    public function testContainsRejectsAnEquivalentForeignTree(): void
    {
        $tree = Expression::literal(1, Dialect::PostgreSql)->sql;
        self::assertFalse(\SqlSemantics\Model\Transformation\TreeEdit::contains($tree, Expression::literal(1, Dialect::PostgreSql)->sql));
        self::assertTrue(\SqlSemantics\Model\Transformation\TreeEdit::contains($tree, $tree));
    }
    public function testRewriteReplacesTheRoot(): void
    {
        $tree = Expression::literal(1, Dialect::PostgreSql)->sql;
        $replacement = Expression::literal(2, Dialect::PostgreSql)->sql;
        self::assertSame($replacement, \SqlSemantics\Model\Transformation\TreeEdit::rewrite($tree, $tree, $replacement));
    }
    public function testReplaceRejectsForeignOwnership(): void
    {
        $tree = Expression::literal(1, Dialect::PostgreSql)->sql;
        $this->expectException(InvalidStructure::class);
        \SqlSemantics\Model\Transformation\TreeEdit::replace($tree, Expression::literal(1, Dialect::PostgreSql)->sql, $tree);
    }
}
