...
#[\PHPUnit\Framework\Attributes\CoversClass(Sql\Tree::class)]
...
{
    public function testAtomsRetainsNestedOrder(): void
    {
        $first = new Sql\Atom('keyword', 'SELECT');
        $second = new Sql\Atom('number', '1');
        $tree = new Sql\Tree('statement', [$first, new Sql\Tree('expression', [$second])]);
        self::assertSame([$first, $second], $tree->atoms());
    }
    public function testToStringProtectsTerminalBoundaries(): void
    {
        $tree = new Sql\Tree('expression', [new Sql\Atom('operator', '-'), new Sql\Atom('operator', '-'), new Sql\Atom('number', '1')]);
