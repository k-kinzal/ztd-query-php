...
{
    public function testKeepsSortDirectionSeparateFromTheExpression(): void
    {
        $query = (new AnalysisCase())->query('SELECT id FROM users ORDER BY id ASC NULLS FIRST');
        self::assertFalse($query->orderBy[0]->descending);
        self::assertTrue($query->orderBy[0]->nullsFirst);
    }
