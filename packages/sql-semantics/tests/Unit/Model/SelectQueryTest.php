...
final class SelectQueryTest extends TestCase
{
    ...
    {
        $query = (new AnalysisCase())->query('SELECT 1 FROM users WHERE score > 0');
        self::assertSame([], $query->outputs[0]->expression->lineage());
        self::assertNotNull($query->where);
        self::assertSame('score', $query->where->lineage()[0]->column->name);
