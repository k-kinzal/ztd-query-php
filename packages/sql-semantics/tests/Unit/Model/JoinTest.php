...
{
    public function testRetainsTheMatchTree(): void
    {
        $query = (new AnalysisCase())->query('SELECT a.id FROM users a LEFT JOIN users b ON a.id=b.id');
        self::assertInstanceOf(\SqlSemantics\Model\Join::class, $query->from);
        self::assertSame($query->relations[0], $query->from->left);
        self::assertSame($query->relations[1], $query->from->right);
