...
#[CoversClass(ImportHistogramStatement::class)]
#[Medium]
final class ImportHistogramStatementTest extends TestCase
{
    public function testWithTargetChangesTableAndColumnIdentitiesTogether(): void
    {
        ...
        $statement = $binder->bind("ANALYZE TABLE t UPDATE HISTOGRAM ON id USING DATA '{}'");
        self::assertInstanceOf(ImportHistogramStatement::class, $statement);
        $other = $binder->bind(str_replace('TABLE t ', 'TABLE u ', "ANALYZE TABLE t UPDATE HISTOGRAM ON id USING DATA '{}'"));
        self::assertInstanceOf(ImportHistogramStatement::class, $other);
        $changed = $statement->withTarget($other->table, $other->column);
    ...
    public function testWithTargetRejectsColumnsFromAnotherTable(): void
    {
        ...
        $statement->withTarget($other->table, $statement->column);
    }
    ...
    public function testWithDataKeepsTheImportedPayloadOpaque(): void
    {
        $binder = new Binder((new SchemaBuilder(Dialect::MySql))->build('CREATE TABLE t(id INT, x INT); CREATE TABLE u(id INT, x INT)'));
        $statement = $binder->bind("ANALYZE TABLE t UPDATE HISTOGRAM ON id USING DATA '{}'");
        self::assertInstanceOf(ImportHistogramStatement::class, $statement);
        $query = $binder->bind("SELECT 'uninterpreted histogram payload'");
        self::assertInstanceOf(\SqlSemantics\Model\BoundSelect::class, $query);
        $data = $query->outputs[0]->expression;
        self::assertInstanceOf(\SqlSemantics\Model\Scalar\Value\Literal::class, $data);
        $changed = $statement->withData($data);
        self::assertSame("'{}'", $statement->data->text);
        self::assertSame("'uninterpreted histogram payload'", $changed->data->text);
    }

    public function testWithDataRejectsANumericOperand(): void
    {
        $binder = new Binder((new SchemaBuilder(Dialect::MySql))->build('CREATE TABLE t(id INT, x INT); CREATE TABLE u(id INT, x INT)'));
        $statement = $binder->bind("ANALYZE TABLE t UPDATE HISTOGRAM ON id USING DATA '{}'");
        self::assertInstanceOf(ImportHistogramStatement::class, $statement);
