...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlSemantics\Model\Statement\RelationQuery::class)]
...
final class RelationQueryTest extends TestCase
{
    public function testRetainsFunctionResultDependencies(): void
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::PostgreSql))->build()))->bind('SELECT value FROM generate_series(1,3) AS t(value)');
        self::assertInstanceOf(\SqlSemantics\Model\Statement\RelationQuery::class, $statement->relations[0]->query);
        self::assertSame('value', $statement->outputs[0]->name);
        self::assertSame('GENERATE_SERIES', $statement->relations[0]->query->outputs[0]->expression->symbol);
    }
}
