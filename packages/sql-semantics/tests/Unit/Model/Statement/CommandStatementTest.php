...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlSemantics\Model\Statement\CommandStatement::class)]
...
final class CommandStatementTest extends TestCase
{
    public function testWithStatementRefreshesDependentFacts(): void
    {
        $binder = new Binder((new SchemaBuilder(Dialect::PostgreSql))->build('CREATE TABLE t(id INTEGER, n INTEGER)'));
        $statement = $binder->bind('EXPLAIN SELECT id FROM t');
        self::assertInstanceOf(\SqlSemantics\Model\Statement\CommandStatement::class, $statement);
        $changed = $statement->withStatement($statement->statements[0], $binder->bind('SELECT n FROM t'));
        self::assertSame('n', $changed->statements[0]->outputs[0]->expression->binding?->column->name);
