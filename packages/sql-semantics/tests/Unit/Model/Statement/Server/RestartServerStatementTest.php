...
#[CoversClass(RestartServerStatement::class)]
#[Medium]
final class RestartServerStatementTest extends TestCase
{
    public function testWithOriginRetainsSemanticOperands(): void
    {
        $binder = new Binder((new SchemaBuilder(Dialect::MySql))->build());
        $statement = $binder->bind('RESTART');
        self::assertInstanceOf(RestartServerStatement::class, $statement);
    ...
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('RESTART');
        self::assertInstanceOf(RestartServerStatement::class, $statement);
        $origin = new \SqlSemantics\Model\Statement\Origin('s0', $statement->source, Dialect::Sqlite);
        $this->expectException(\SqlSemantics\Model\Validation\InvalidStructure::class);
        new RestartServerStatement($origin);
    }
}
