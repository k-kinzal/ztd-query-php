...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlSemantics\Model\Statement\CreateIndexStatement::class)]
...
final class CreateIndexStatementTest extends TestCase
{
    public function testWithKeyRefreshesDependentFacts(): void
    {
        $binder = new Binder((new SchemaBuilder(Dialect::PostgreSql))->build('CREATE TABLE t(id INTEGER, n INTEGER)'));
        $statement = $binder->bind('CREATE INDEX ix ON t((id+1))');
        self::assertInstanceOf(\SqlSemantics\Model\Statement\CreateIndexStatement::class, $statement);
        $changed = $statement->withKey(0, Expression::reference(['n'], Dialect::PostgreSql));
        self::assertSame('n', $changed->index->definition->elements[0]->value()->lineage()[0]->column->name);
        self::assertSame('id', $statement->index->definition->elements[0]->value()->lineage()[0]->column->name);
    }
