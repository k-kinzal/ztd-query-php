...
#[CoversClass(DropLogfileGroupStatement::class)]
#[Medium]
final class DropLogfileGroupStatementTest extends TestCase
{
    public function testWithNamePreservesTheOriginalAndQuotesTheReplacement(): void
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('DROP LOGFILE GROUP store');
        self::assertInstanceOf(DropLogfileGroupStatement::class, $statement);
    ...
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('DROP LOGFILE GROUP store ENGINE NDB');
        self::assertInstanceOf(DropLogfileGroupStatement::class, $statement);
