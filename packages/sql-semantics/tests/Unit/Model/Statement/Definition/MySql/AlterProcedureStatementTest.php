...
#[CoversClass(AlterProcedureStatement::class)]
#[Medium]
final class AlterProcedureStatementTest extends TestCase
{
    public function testWithNameChangesOnlyTheTargetAndQuotesItsParts(): void
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('ALTER PROCEDURE app.r NO SQL');
    ...
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('ALTER PROCEDURE r');
        self::assertInstanceOf(AlterProcedureStatement::class, $statement);
    ...
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('ALTER PROCEDURE r NO SQL');
        self::assertInstanceOf(AlterProcedureStatement::class, $statement);
        ...
        self::assertSame('ALTER PROCEDURE `r`', $changed->withChanges(new RoutineAlteration())->toString());
    }

    public function testWithOriginRetainsTheCompleteRequest(): void
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('ALTER PROCEDURE r MODIFIES SQL DATA');
        self::assertInstanceOf(AlterProcedureStatement::class, $statement);
    ...
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql, grammarVersion: 'mysql-8.0.44'))->build()))->bind('ALTER PROCEDURE r');
        self::assertInstanceOf(AlterProcedureStatement::class, $statement);
        $this->expectException(InvalidStructure::class);
