...
#[CoversClass(Statement\CreateObjectsPublicationStatement::class)]
#[Medium]
final class CreateObjectsPublicationStatementTest extends TestCase
{
    ...
    public function testWithObjectsRequiresAnObject(): void
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::PostgreSql))->build('CREATE TABLE t(a INT, b INT)')))->bind('CREATE PUBLICATION p FOR TABLE t');
        self::assertInstanceOf(Statement\CreateObjectsPublicationStatement::class, $statement);
        self::assertSame('CREATE PUBLICATION "p" FOR TABLES IN SCHEMA "s"', $statement->withObjects([new Operand\PublishedSchema('s')])->toString());
        $this->expectException(InvalidStructure::class);
