...
#[CoversClass(\SqlSemantics\Model\Statement\Definition\PostgreSql\DropAggregatesStatement::class)]
#[Medium]
final class DropAggregatesStatementTest extends TestCase
{
    public function testWithTargetsReplacesTheRequestWithoutMutatingItsSource(): void
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::PostgreSql))->build()))->bind('DROP AGGREGATE f(*)');
        self::assertInstanceOf(\SqlSemantics\Model\Statement\Definition\PostgreSql\DropAggregatesStatement::class, $statement);
        $changed = $statement->withTargets([new Routine\ZeroArgumentAggregate(new QualifiedName(['app', 'g']))]);
        self::assertSame(['f'], $statement->targets[0]->name->parts);
        self::assertSame('DROP AGGREGATE "app"."g"(*)', $changed->toString());
