...
final class UpdateJoinedStatementTest extends TestCase
{
    public function testBindsTheJoinedInputAndModifiers(): void
    {
        ...
        self::assertSame('UPDATE LOW_PRIORITY IGNORE `t` INNER JOIN `s` ON (`t`.`id` = `s`.`id`) SET `t`.`n` = `s`.`n` WHERE (`t`.`id` > 0)', (new \SqlSemantics\SimpleSerializer())->serialize($statement));
    }
    ...
    public function testAffectedTablesListsOnlyTheAssignedTargets(): void
    {
        ...
        self::assertSame('UPDATE `t` CROSS JOIN `s` SET `t`.`n` = `s`.`n` WHERE (`t`.`id` = `s`.`id`)', (new \SqlSemantics\SimpleSerializer())->serialize($statement));
    }
    ...
    public function testWithWhereAddsAPredicateImmutably(): void
    {
        ...
        self::assertSame('UPDATE `t` INNER JOIN `s` ON (`t`.`id` = `s`.`id`) SET `t`.`n` = `s`.`n` WHERE (`t`.`id` > 1)', (new \SqlSemantics\SimpleSerializer())->serialize($changed));
    ...
    public function testWithAssignmentsReplacesTheWritesImmutably(): void
    {
        ...
        self::assertSame('UPDATE `t` INNER JOIN `s` ON (`t`.`id` = `s`.`id`) SET `t`.`id` = `s`.`id`', (new \SqlSemantics\SimpleSerializer())->serialize($changed));
