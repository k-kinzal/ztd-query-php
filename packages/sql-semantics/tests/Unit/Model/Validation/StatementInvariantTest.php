...
#[\PHPUnit\Framework\Attributes\CoversClass(\SqlSemantics\Model\Validation\StatementInvariant::class)]
...
final class StatementInvariantTest extends TestCase
{
    ...
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::PostgreSql))->build()))->bind('WITH q AS (SELECT 1 AS n) SELECT n FROM q');
        \SqlSemantics\Model\Validation\StatementInvariant::collections($statement);
        self::assertSame(['q'], array_keys($statement->ctes));
