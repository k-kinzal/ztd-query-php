...
#[CoversClass(QualifiedInfixOperation::class)]
#[Medium]
final class QualifiedInfixOperationTest extends TestCase
{
    public function testInputsListTheLeftOperandBeforeTheRight(): void
    {
        $binder = new Binder((new SchemaBuilder(Dialect::PostgreSql))->build('CREATE TABLE t (n INTEGER)'));
        $query = $binder->bind('SELECT n OPERATOR(public.<->) 2 FROM t');
        self::assertInstanceOf(BoundSelect::class, $query);
        $operation = $query->outputs[0]->expression;
        self::assertInstanceOf(QualifiedInfixOperation::class, $operation);
        ...
        self::assertSame('SELECT ("n" OPERATOR("public".<->) 2) FROM "public"."t"', (new \SqlSemantics\SimpleSerializer())->serialize($query));
    ...
    public function testSpellingNamesTheOperator(): void
    {
        ...
        $operation = new QualifiedInfixOperation($value->facts, $value->source, new QualifiedOperator(['geo'], '<->'), $value, $value);
    ...
    {
        $value = Expression::literal(1, Dialect::PostgreSql);
        $operation = new QualifiedInfixOperation($value->facts, $value->source, new QualifiedOperator([], '#'), $value, $value);
    ...
    public function testInputsRequirePostgreSqlOperands(): void
    {
        ...
        new QualifiedInfixOperation($value->facts, $value->source, new QualifiedOperator([], '#'), $value, $value);
    }
}
