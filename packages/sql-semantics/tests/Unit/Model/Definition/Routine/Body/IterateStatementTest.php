...
#[CoversClass(IterateStatement::class)]
#[Medium]
final class IterateStatementTest extends TestCase
{
    public function testNamesTheEnclosingLoop(): void
    {
        $statement = (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('CREATE PROCEDURE p() l: LOOP ITERATE l; END LOOP');
        self::assertInstanceOf(CreateProcedureStatement::class, $statement);
        self::assertInstanceOf(LoopStatement::class, $statement->body);
        self::assertInstanceOf(IterateStatement::class, $statement->body->statements[0]);
    ...
    public function testDiagnosesAnUnknownLabel(): void
    {
        ...
        (new Binder((new SchemaBuilder(Dialect::MySql))->build()))->bind('CREATE PROCEDURE p() l: LOOP ITERATE m; END LOOP', strict: false);
    }
}
