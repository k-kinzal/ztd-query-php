...
final class SelectReaderTest extends TestCase
{
    ...
    public function testReadConstantQueriesWithoutARelation(Dialect $dialect): void
    {
        $query = (new AnalysisCase($dialect))->query('SELECT DISTINCT 1 AS one');
        self::assertNull($query->from);
        self::assertSame([], $query->relations);
