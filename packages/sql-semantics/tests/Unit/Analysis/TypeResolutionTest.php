...
#[Medium]
final class TypeResolutionTest extends TestCase
{
    ...
    public function testRejectsUnmodeledCoercion(): void
    {
        ...
        (new AnalysisCase())->query('SELECT COALESCE(id, TRUE) FROM users');
    }


    public function testBooleanUsesDialectSpecificResultTypes(): void
    {
        self::assertSame('boolean', (new \SqlSemantics\Analysis\TypeResolution(Dialect::PostgreSql))->boolean()->name);
        self::assertSame('integer', (new \SqlSemantics\Analysis\TypeResolution(Dialect::MySql))->boolean()->name);
        self::assertSame('integer', (new \SqlSemantics\Analysis\TypeResolution(Dialect::Sqlite))->boolean()->name);
    }
}
