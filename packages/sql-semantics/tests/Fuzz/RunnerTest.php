...
#[\PHPUnit\Framework\Attributes\CoversClass(\Fuzz\Runner::class)]
...
{
    /**
     * Checks that findings become failing process statuses.
     */
    #[\PHPUnit\Framework\Attributes\DataProvider('providerStatuses')]
    public function testStatusFailsOnFindingsEvenWhenTheProcessExitsZero(int $exitCode, string $output, int $expected): void
    {
        self::assertSame($expected, (new \Fuzz\Runner())->status($exitCode, $output));
    }
    ...
    {
        yield 'success' => [0, 'NEW run: 300', 0];
        yield 'finding' => [0, 'CRASH in crash-input.txt', 1];
        yield 'corpus finding' => [0, 'CORPUS CRASH', 1];
        yield 'instrumentation failure' => [0, 'PARSE ERROR', 1];
        yield 'process failure' => [255, 'Fatal error', 1];
    }
}
