...
#[\PHPUnit\Framework\Attributes\CoversClass(\Fuzz\Target\SchemaProperties::class)]
...
final class SchemaPropertiesTest extends TestCase
{
    /**
     * The companion oracle varies storage order and values without restricting grammar generation.
     */
    public function testVerifyMysqlAndPostgresFacts(): void
    {
        $mysql = new \Fuzz\Target\SchemaProperties(Dialect::MySql, 'mysql-5.6.51');
        $postgres = new \Fuzz\Target\SchemaProperties(Dialect::PostgreSql, 'pg-17.2');
        $mysql->verify("\0\xff");
        $postgres->verify("\1\x80");
        self::assertSame(Dialect::MySql, $mysql->dialect);
        self::assertSame(Dialect::PostgreSql, $postgres->dialect);
    }
    /**
     * SQLite retains dynamic typing while still enforcing names and storage positions.
     */
    public function testVerifySqliteFacts(): void
    {
        $target = new \Fuzz\Target\SchemaProperties(Dialect::Sqlite, 'sqlite-3.47.2');
        $target->verify('');
        self::assertSame(Dialect::Sqlite, $target->dialect);
    }
}
