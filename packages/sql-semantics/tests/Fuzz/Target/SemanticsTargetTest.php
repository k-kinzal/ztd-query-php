...
#[\PHPUnit\Framework\Attributes\CoversClass(\Fuzz\Target\SemanticsTarget::class)]
...
final class SemanticsTargetTest extends TestCase
{
    /**
     * Checks that the property harness detects the specified invariant violation.
     */
    public function testVerifyProcessesUnresolvedGraphs(): void
    {
        $target = new \Fuzz\Target\SemanticsTarget(new Binder((new SchemaBuilder(Dialect::PostgreSql))->build()));
        $target->verify('WITH q AS (TABLE missing) SELECT q.id FROM q WHERE q.id>1', 'seed');
        self::assertSame([], $target->binder->schema->tables);
    }

    /**
     * Checks that the property harness detects the specified invariant violation.
     */
    public function testVerifyMakesSyntaxRejectionsReplayableFailures(): void
    {
        $target = new \Fuzz\Target\SemanticsTarget(new Binder((new SchemaBuilder(Dialect::PostgreSql))->build()));
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage("Input hex: 73656564\nSQL:\nSELECT (");
        $target->verify('SELECT (', 'seed');
    }
}
