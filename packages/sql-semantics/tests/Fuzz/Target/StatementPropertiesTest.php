...
#[\PHPUnit\Framework\Attributes\CoversClass(\Fuzz\Target\StatementProperties::class)]
#[\PHPUnit\Framework\Attributes\UsesClass(\Fuzz\Target\GraphProperties::class)]
...
final class StatementPropertiesTest extends TestCase
{
    /**
     * Omitting a required meaning must fail even when the original SQL was retained.
     */
    public function testVerifyRejectsMissingConfigurationEffects(): void
    {
        $valid = (new Binder((new SchemaBuilder(Dialect::PostgreSql))->build()))->bind('SET LOCAL work_mem TO DEFAULT');
        $broken = new \SqlSemantics\Model\BoundStatement('s0', null, [], [], null, false, [], null, null, $valid->source, kind: 'SET');
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('named effects');
        (new \Fuzz\Target\StatementProperties())->verify($broken);
    }
    /**
     * Source retention cannot substitute for INSERT positional meaning.
     */
    public function testInsertionRejectsMissingInputMapping(): void
    {
        $valid = (new Binder((new SchemaBuilder(Dialect::PostgreSql))->build()))->analyze('INSERT INTO t(id) VALUES(1)')->statement;
        $broken = new \SqlSemantics\Model\BoundStatement('s0', null, [], [], null, false, [], null, null, $valid->source, kind: 'INSERT');
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('input-to-column mapping');
        (new \Fuzz\Target\StatementProperties())->verify($broken);
    }
    /**
     * A compound operator owns branches; its branch owns the filter.
     */
    public function testVerifyChecksCompoundPredicatesInTheirOwnScope(): void
    {
        $valid = (new Binder((new SchemaBuilder(Dialect::MySql, grammarVersion: 'mysql-5.6.51'))->build()))->bind('SELECT 1 FROM DUAL WHERE 1=1 UNION SELECT 2');
        (new \Fuzz\Target\GraphProperties(Dialect::MySql))->statement($valid);
        self::assertNull($valid->where);
        self::assertSame('=', $valid->branches[0]->where?->symbol);
    }
}
