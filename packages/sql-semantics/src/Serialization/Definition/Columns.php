...
final class Columns
{
     ...
     * the unique key its caller leaves out, and writes AUTO_INCREMENT once that key is gone.
     * @param ConstraintResponse $keyConflict ON CONFLICT resolution of the SQLite primary key an AUTOINCREMENT column declares
     * @param bool $serialKey Whether the unique key SERIAL DEFAULT VALUE declares is still present
     */
    public static function write(ColumnDefinition $column, Dialect $dialect, ConstraintResponse $keyConflict = ConstraintResponse::Default, bool $serialKey = true): Tree
    {
        ...
        $serial = $serialKey && $generation instanceof Column\AutoIncrementColumn && $generation->serialDefault;
        ...
        $parts = [Build::identifier([$column->name], $dialect), self::type($column), ...$computed, ColumnAttributes::write($column->attributes, $dialect)];
        ...
        array_push($parts, ...self::generation($generation, $dialect, $keyConflict, $serial, $computed === []));
        if ($trailingNull) {
            $parts[] = Build::keyword('NULL');
        }
    ...
    /**
     * Returns the unique key MySQL SERIAL DEFAULT VALUE declared on the column, which the column writes with that
     * attribute: the first unnamed key on the column alone without key options; null for any other column.
    ...
    {
        if (!$column->generation instanceof Column\AutoIncrementColumn || !$column->generation->serialDefault) {
            return null;
        }
