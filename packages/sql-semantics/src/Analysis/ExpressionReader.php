...
final class ExpressionReader
{
     ...
     */
    public function read(Node|Token $node, Scope $scope): Expression
    {
    ...
    {
        $literal = (new LiteralReader($scope->identifiers->dialect))->read($token);
        if ($literal !== null) {
            return $literal;
        }
    ...
    public function operation(Node $node, array $children, Scope $scope): ?Expression
    {
        ...
        if (count($children) === 2 && in_array(strtoupper(Tree::text($children[0])), ['+', '-', 'NOT'], true)) {
            return $rules->operator(Tree::text($children[0]), [$this->read($children[1], $scope)], $node);
        }
        if (count($children) >= 2) {
            ...
            if (in_array($tail, ['IS NULL', 'IS NOT NULL', 'ISNULL', 'NOTNULL'], true)) {
                return $rules->operator(in_array($tail, ['IS NULL', 'ISNULL'], true) ? 'IS NULL' : 'IS NOT NULL', [$this->read($children[0], $scope)], $node);
            }
        }
        if (count($children) === 3 && in_array(strtoupper(Tree::text($children[1])), ['+', '-', '*', '=', '<>', '!=', '<', '>', '<=', '>=', 'AND', 'OR', 'IS', '<=>'], true)) {
            return $rules->operator(Tree::text($children[1]), [$this->read($children[0], $scope), $this->read($children[2], $scope)], $node);
        }
