...
final class TailReader
{
    ...
    public function sortExpression(Node $node, Scope $scope, array $outputs): Expression
    {
        ...
        return (new ExpressionReader())->read($node, $scope);
    }
    ...
    public function pagination(Node $statement, Scope $scope): array
    {
        ...
        $bound = array_map(static fn (Node $node): Expression => (new ExpressionReader())->read($node, $scope), $expressions);
        if (count($bound) > 2) {
        ...
        if ($offset !== null) {
            ...
            return [$bound[0] ?? null, (new ExpressionReader())->read($node, $scope)];
        }
