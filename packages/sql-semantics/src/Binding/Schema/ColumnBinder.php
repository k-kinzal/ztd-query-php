...
final class ColumnBinder
{
    ...
    public static function bind(ParsedColumn $column, Scope $scope): ColumnDefinition
    {
        ...
        OptionBinding::classified($options, ['collation', 'character_set', 'comment', 'invisible', 'visible', 'storage', 'column_format', 'compression', 'engine_attribute', 'secondary_engine_attribute', 'srid', 'zerofill', 'binary', 'signed', 'unsigned', 'auto_increment', 'serial_default', 'serial_type', 'serial', 'identity', 'start', 'increment', 'minvalue', 'maxvalue', 'cache', 'cycle', 'no', 'as', 'sequence', 'restart', 'owned', 'logged', 'unlogged', 'generated_storage', 'on_update', 'not_secondary']);
    ...
    public static function generation(ParsedColumn $column, Scope $scope): Column\Generation
    {
        if (isset($column->options['auto_increment'])) {
            ...
            return new Column\AutoIncrementColumn(isset($column->options['serial_default']) && !isset($column->options['serial_type']), isset($column->options['serial_type']));
        }
