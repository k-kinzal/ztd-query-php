...
final class ExpressionBinder
{
    ...
    public function bind(Node|Token $node, Scope $scope): Expression
    {
        ...
        if (in_array($node->name, $scope->identifiers->dialect->platform()->syntax()->nodes('columnReference'), true)) {
        ...
        if (in_array($node->name, $scope->identifiers->dialect->platform()->syntax()->nodes('qualifiedExpression'), true) && $this->qualified($children, $scope->identifiers->dialect->platform()->syntax())) {
            return $scope->column($scope->identifiers->parts($node), $node);
        }
    ...
    public function token(Token $token, Scope $scope): Expression
    {
        ...
        if (in_array($token->name, $scope->identifiers->dialect->platform()->syntax()->nodes('identifierToken'), true)) {
     ...
     */
    public function qualified(array $children, \SqlSemantics\Core\Policy\SyntaxRules $syntax): bool
    {
    ...
    public function call(Node $node, array $children, Scope $scope): Expression
    {
        ...
        foreach (array_slice($children, 2, -1) as $child) {
            if ($child instanceof Node) {
                foreach (Tree::outer($child, $scope->identifiers->dialect->platform()->syntax()->nodes('expression')) as $argument) {
