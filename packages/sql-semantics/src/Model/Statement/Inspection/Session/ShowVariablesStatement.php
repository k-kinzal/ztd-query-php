...
/**
 * Lists the server's system variables with their values in one scope.
 * @visibility public
 * @example Inspecting the scope and restriction
 *     $statement = (new \SqlSemantics\Binder((new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::MySql))->build()))->bind("SHOW GLOBAL VARIABLES LIKE 'max%'");
 *     [$statement->scope->value, $statement->filter instanceof \SqlSemantics\Model\Query\Inspection\Filter\PatternFilter] // => ['GLOBAL', true]
 */
final class ShowVariablesStatement extends InspectionStatement
{
    /**
