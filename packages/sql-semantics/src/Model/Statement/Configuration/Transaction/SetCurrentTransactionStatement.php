...
/**
 * Requests ordered PostgreSQL mode changes for the current transaction.
 * @visibility public
 ...
 *     $statement instanceof \SqlSemantics\Model\Statement\Configuration\Transaction\SetCurrentTransactionStatement // => true
 * @example Rejecting an empty mode request
 *     $statement = (new \SqlSemantics\Binder((new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::PostgreSql))->build()))->bind("SET TRANSACTION READ ONLY");
 *     new \SqlSemantics\Model\Statement\Configuration\Transaction\SetCurrentTransactionStatement($statement->origin, []); // throws \SqlSemantics\Model\Validation\InvalidStructure
 */
final class SetCurrentTransactionStatement extends ConfigurationStatement
{
    /**
    ...
    public function __construct(Origin $origin, public readonly array $modes, public readonly Locality $locality = Locality::Session)
    {
        if ($origin->dialect !== Dialect::PostgreSql) {
            throw new InvalidStructure('SetCurrentTransactionStatement requires PostgreSql.');
        }
