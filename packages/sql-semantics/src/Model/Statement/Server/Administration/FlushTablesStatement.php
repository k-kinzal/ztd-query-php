...
/**
 * Closes and reopens the named tables, or every open table when none is named (FLUSH TABLES).
 * @visibility public
 * @example Binding the operation
 *     $statement = (new \SqlSemantics\Binder((new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::MySql))->build('CREATE TABLE t(id INT)')))->bind('FLUSH LOCAL TABLES t');
 *     $statement->binlog->value // => 'NO_WRITE_TO_BINLOG'
 */
final class FlushTablesStatement extends BoundStatement
{
    /**
    ...
    {
        $this->tables = FlushedTables::check($origin, $tables, 'FLUSH TABLES');
