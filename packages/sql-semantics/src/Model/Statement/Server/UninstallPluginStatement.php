...
/**
 * Removes one named server plugin.
 * @example Binding the operation
 *     $schema = (new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::MySql))->build();
 *     $statement = (new \SqlSemantics\Binder($schema))->bind('UNINSTALL PLUGIN audit');
 *     $statement instanceof \SqlSemantics\Model\Statement\Server\UninstallPluginStatement // => true
 * @visibility public
 */
final class UninstallPluginStatement extends BoundStatement
{
    /**
    ...
    public function __construct(Origin $origin, public readonly string $name)
    {
        if ($origin->dialect !== Dialect::MySql) {
            throw new InvalidStructure('UninstallPluginStatement requires MySql.');
        }
