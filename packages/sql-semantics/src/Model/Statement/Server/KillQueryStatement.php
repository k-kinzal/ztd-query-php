...
/**
 * Interrupts the active query on the connection identified by an expression.
 * @example Binding the operation
 *     $schema = (new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::MySql))->build();
 *     $statement = (new \SqlSemantics\Binder($schema))->bind('KILL QUERY 42');
 *     $statement instanceof \SqlSemantics\Model\Statement\Server\KillQueryStatement // => true
 * @visibility public
 */
final class KillQueryStatement extends BoundStatement
{
    /**
    ...
    public function __construct(Origin $origin, public readonly Expression $connectionId)
    {
        if ($origin->dialect !== Dialect::MySql) {
            throw new InvalidStructure('KillQueryStatement requires MySql.');
        }
