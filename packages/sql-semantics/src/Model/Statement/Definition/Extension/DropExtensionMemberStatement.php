...
/**
 * Releases an object from an extension, so that it is no longer dropped or dumped with the extension.
 * @visibility public
 * @example Reading the DROP member
 *     $statement = (new \SqlSemantics\Binder((new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::PostgreSql))->build()))->bind('ALTER EXTENSION hstore DROP TABLE app.t');
 *     $statement->extension // => 'hstore'
 *     $statement->object->kind->value // => 'TABLE'
 * @example Rejecting a column as a DROP member
 *     $statement = (new \SqlSemantics\Binder((new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::PostgreSql))->build()))->bind('ALTER EXTENSION hstore DROP TABLE app.t');
 ...
 */
final class DropExtensionMemberStatement extends BoundStatement
{
    /**
