...
/**
 * Describes removal of one named MySQL event without executing it.
 * @visibility public
 * @example Inspecting the deletion target
 *     $statement = (new \SqlSemantics\Binder((new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::MySql))->build()))->bind('DROP EVENT app.e');
 *     $statement->name->parts // => ['app', 'e']
 */
final class DropEventStatement extends BoundStatement
{
    /**
    ...
    public function __construct(Origin $origin, public readonly QualifiedName $name, public readonly bool $ifExists = false)
    {
        ...
        if (count($name->parts) > 2) {
            throw new InvalidStructure('An event name is local or database-qualified.');
        }
