...
/**
 * Requests ALTER TABLESPACE that removes a data file from the tablespace, without touching the file.
 * @visibility public
 * @example Inspecting the removed data file
 *     $statement = (new \SqlSemantics\Binder((new \SqlSemantics\SchemaBuilder(\SqlSemantics\Dialect::MySql))->build()))->bind("ALTER TABLESPACE ts DROP DATAFILE 'old.dat' NO_WAIT");
 *     [$statement->datafile, $statement->changes->waiting->value] // => ['old.dat', 'NO_WAIT']
 */
final class DropTablespaceDatafileStatement extends BoundStatement
{
    /**
