...
SQL Semantics converts SQL strings into structured declarations and bound statements for MySQL, PostgreSQL, and SQLite. It uses [sql-parser](../sql-parser/) to preserve the original SQL and exposes table and column definitions, types, nullability, query inputs, write destinations, conditions, and configuration effects. No database connection is required. See the [schema API](docs/schema.md), [binder API](docs/binder.md), and [limitations](docs/limitation.md).
...
$statement->toSql(); // SELECT id, score FROM users WHERE score > 0
```

`SchemaBuilder::build()` returns an independent schema snapshot. `Binder::bind()` returns a bound statement, and `Binder::analyze()` returns the same structure with diagnostics when names or types cannot be resolved. `Binder::replaceExpression()` reparses and rebinds an expression edit into a new snapshot. The API tables and SQL examples are in [schema.md](docs/schema.md) and [binder.md](docs/binder.md).
