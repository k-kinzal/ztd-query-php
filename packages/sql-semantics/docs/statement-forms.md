...
| PostgreSQL and SQLite: `VALUES (1), (2)`; MySQL: `VALUES ROW(1), ROW(2)` | `ValuesStatement` | Nonempty `rows` of equal width; result columns and common types are derived from those rows. |
| MySQL and PostgreSQL: `TABLE users` | `TableStatement` | A required table or CTE reference; result columns are derived from that declaration. |
