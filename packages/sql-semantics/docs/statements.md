...
The compiler reads the official Bison/Lemon grammars for all releases registered
by sql-parser: MySQL 5.6.51, 5.7.44, 8.0.44, 8.1.0, 8.2.0, 8.3.0, 8.4.7, 9.0.1,
...
releases. Generation always processes the whole release set together.

`resources/models` contains the parser-independent definitions.
`resources/mapping` contains version-specific construction recipes, loaded only
by `Core/Analysis/ValueReader`. During analysis the reader visits the transient
...
Generation stages its output under `build/model-resources`, then installs changed
...
The [round-trip fuzz property](../fuzz/README.md) checks reconstruction using all
