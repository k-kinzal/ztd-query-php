...
final class InsertDelayedTest extends MySqlIntegrationTestCase
{
    public function testInsertDelayedIsSupported(): void
    {
        $table = $this->uniqueTableName('users');

        $this->rawPdo->exec("CREATE TABLE `{$table}` (id INT PRIMARY KEY, name VARCHAR(255))");

        $this->ztdPdo->exec("INSERT DELAYED INTO `{$table}` (id, name) VALUES (1, 'Alice')");
