...
final class LoadDataInfileTest extends MySqlIntegrationTestCase
{
    public function testLoadDataInfileThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("LOAD DATA INFILE '/tmp/data.csv' INTO TABLE `{$table}`");
    }

    public function testLoadDataInfileIsIgnoredInIgnoreMode(): void
    {
        $ztdPdo = $this->createZtdPdoWithConfig(new ZtdConfig(
        ...
        $result = $ztdPdo->exec("LOAD DATA INFILE '/tmp/data.csv' INTO TABLE `{$table}`");

        $this->assertSame(0, $result);
    }

    public function testLoadDataInfileTriggersNoticeInNoticeMode(): void
    {
        $ztdPdo = $this->createZtdPdoWithConfig(new ZtdConfig(
