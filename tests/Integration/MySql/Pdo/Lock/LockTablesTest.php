...
final class LockTablesTest extends MySqlIntegrationTestCase
{
    public function testLockTablesThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("LOCK TABLES `{$table}` READ");
    }

    public function testLockTablesIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->exec("LOCK TABLES `{$table}` READ");

        $this->assertSame(0, $result);
    }

    public function testLockTablesTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
