...
final class LockTablesWriteTest extends MySqlIntegrationTestCase
{
    public function testLockTablesWriteThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("LOCK TABLES `{$table}` WRITE");
    }

    public function testLockTablesWriteIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->exec("LOCK TABLES `{$table}` WRITE");

        $this->assertSame(0, $result);
    }

    public function testLockTablesWriteTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('LOCK TABLES', $capturedNotices[0]);
        } finally {
