...
final class RepairTableTest extends MySqlIntegrationTestCase
{
    public function testRepairTableThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("REPAIR TABLE `{$table}`");
    }

    public function testRepairTableIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("REPAIR TABLE `{$table}`");

        $this->assertFalse($result);
    }

    public function testRepairTableTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('REPAIR', $capturedNotices[0]);
        } finally {
