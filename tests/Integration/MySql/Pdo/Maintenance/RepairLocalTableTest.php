...
final class RepairLocalTableTest extends MySqlIntegrationTestCase
{
    public function testRepairLocalTableThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("REPAIR LOCAL TABLE `{$table}`");
    }

    public function testRepairLocalTableIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("REPAIR LOCAL TABLE `{$table}`");

        $this->assertFalse($result);
    }

    public function testRepairLocalTableTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('REPAIR LOCAL TABLE', $capturedNotices[0]);
        } finally {
