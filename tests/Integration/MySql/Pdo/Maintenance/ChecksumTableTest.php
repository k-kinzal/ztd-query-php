...
final class ChecksumTableTest extends MySqlIntegrationTestCase
{
    public function testChecksumTableThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("CHECKSUM TABLE `{$table}`");
    }

    public function testChecksumTableIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("CHECKSUM TABLE `{$table}`");

        $this->assertFalse($result);
    }

    public function testChecksumTableTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('CHECKSUM', $capturedNotices[0]);
        } finally {
