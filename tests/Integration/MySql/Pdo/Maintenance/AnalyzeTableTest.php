...
final class AnalyzeTableTest extends MySqlIntegrationTestCase
{
    public function testAnalyzeTableThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("ANALYZE TABLE `{$table}`");
    }

    public function testAnalyzeTableIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("ANALYZE TABLE `{$table}`");

        $this->assertFalse($result);
    }

    public function testAnalyzeTableTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('ANALYZE', $capturedNotices[0]);
        } finally {
