...
final class CheckTableTest extends MySqlIntegrationTestCase
{
    public function testCheckTableThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("CHECK TABLE `{$table}`");
    }

    public function testCheckTableIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("CHECK TABLE `{$table}`");

        $this->assertFalse($result);
    }

    public function testCheckTableTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('CHECK', $capturedNotices[0]);
        } finally {
