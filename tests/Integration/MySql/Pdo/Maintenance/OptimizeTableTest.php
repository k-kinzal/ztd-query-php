...
final class OptimizeTableTest extends MySqlIntegrationTestCase
{
    public function testOptimizeTableThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("OPTIMIZE TABLE `{$table}`");
    }

    public function testOptimizeTableIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("OPTIMIZE TABLE `{$table}`");

        $this->assertFalse($result);
    }

    public function testOptimizeTableTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('OPTIMIZE', $capturedNotices[0]);
        } finally {
