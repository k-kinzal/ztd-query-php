...
final class AlterTableEngineTest extends MySqlIntegrationTestCase
{
    public function testAlterTableEngineThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("ALTER TABLE `{$table}` ENGINE = MyISAM");
    }

    public function testAlterTableEngineIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->exec("ALTER TABLE `{$table}` ENGINE = MyISAM");

        $this->assertSame(0, $result);
    }

    public function testAlterTableEngineTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
