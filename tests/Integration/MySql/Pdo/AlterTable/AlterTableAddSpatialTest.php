...
final class AlterTableAddSpatialTest extends MySqlIntegrationTestCase
{
    public function testAlterTableAddSpatialThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("ALTER TABLE `{$table}` ADD SPATIAL INDEX sp_coords (coords)");
    }

    public function testAlterTableAddSpatialIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('locations');
        ...
        $result = $ztdPdo->exec("ALTER TABLE `{$table}` ADD SPATIAL INDEX sp_coords (coords)");

        $this->assertSame(0, $result);
    }

    public function testAlterTableAddSpatialTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('locations');
