...
final class AlterTableRenameKeyTest extends MySqlIntegrationTestCase
{
    public function testAlterTableRenameKeyThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("ALTER TABLE `{$table}` RENAME KEY key_name TO key_full_name");
    }

    public function testAlterTableRenameKeyIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->exec("ALTER TABLE `{$table}` RENAME KEY key_name TO key_full_name");

        $this->assertSame(0, $result);
    }

    public function testAlterTableRenameKeyTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
