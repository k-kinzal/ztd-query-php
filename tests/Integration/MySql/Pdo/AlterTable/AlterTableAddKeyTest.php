...
final class AlterTableAddKeyTest extends MySqlIntegrationTestCase
{
    public function testAlterTableAddKeyThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("ALTER TABLE `{$table}` ADD KEY key_name (name)");
    }

    public function testAlterTableAddKeyIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->exec("ALTER TABLE `{$table}` ADD KEY key_name (name)");

        $this->assertSame(0, $result);
    }

    public function testAlterTableAddKeyTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
