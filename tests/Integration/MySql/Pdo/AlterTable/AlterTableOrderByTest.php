...
final class AlterTableOrderByTest extends MySqlIntegrationTestCase
{
    public function testAlterTableOrderByThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("ALTER TABLE `{$table}` ORDER BY name");
    }

    public function testAlterTableOrderByIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->exec("ALTER TABLE `{$table}` ORDER BY name");

        $this->assertSame(0, $result);
    }

    public function testAlterTableOrderByTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
