...
final class AlterTableAddIndexTest extends MySqlIntegrationTestCase
{
    public function testAlterTableAddIndexThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->exec("ALTER TABLE `{$table}` ADD INDEX idx_name (name)");
    }

    public function testAlterTableAddIndexIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->exec("ALTER TABLE `{$table}` ADD INDEX idx_name (name)");

        $this->assertSame(0, $result);
    }

    public function testAlterTableAddIndexTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
