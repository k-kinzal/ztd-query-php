...
final class SelectIntoOutfileTest extends MySqlIntegrationTestCase
{
    public function testSelectIntoOutfileThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("SELECT * FROM `{$table}` INTO OUTFILE '/tmp/test.csv'");
    }

    public function testSelectIntoOutfileIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("SELECT * FROM `{$table}` INTO OUTFILE '/tmp/test.csv'");

        $this->assertFalse($result);
    }

    public function testSelectIntoOutfileTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('INTO OUTFILE', $capturedNotices[0]);
        } finally {
