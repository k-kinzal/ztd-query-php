...
final class SelectUnionTest extends MySqlIntegrationTestCase
{
    public function testSelectUnionCombinesResultsWithoutDuplicates(): void
    {
        ...
        $rawRows = $this->rawQuery("SELECT name FROM `{$table1}` UNION SELECT name FROM `{$table2}` ORDER BY name");
        $ztdRows = $this->ztdQuery("SELECT name FROM `{$table1}` UNION SELECT name FROM `{$table2}` ORDER BY name");

        $this->assertSame($rawRows, $ztdRows);
        $this->assertCount(3, $ztdRows); // Bob appears only once
    }
}
