...
final class SelectIntoVariableTest extends MySqlIntegrationTestCase
{
    public function testSelectIntoVariableThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("SELECT name INTO @myvar FROM `{$table}` WHERE id = 1");
    }

    public function testSelectIntoVariableIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("SELECT name INTO @myvar FROM `{$table}` WHERE id = 1");

        $this->assertFalse($result);
    }

    public function testSelectIntoVariableTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('INTO', $capturedNotices[0]);
        } finally {
