...
final class SelectExceptTest extends MySqlIntegrationTestCase
{
    public function testSelectExceptThrowsExceptionInExceptionMode(): void
    {
        ...
        $this->rawPdo->exec("INSERT INTO `{$table2}` VALUES (1, 'Bob')");
        ...
        $ztdPdo->query("SELECT name FROM `{$table1}` EXCEPT SELECT name FROM `{$table2}` ORDER BY name");
    }

    public function testSelectExceptIsIgnoredInIgnoreMode(): void
    {
        $table1 = $this->uniqueTableName('users1');
        $table2 = $this->uniqueTableName('users2');
        ...
        $result = $ztdPdo->query("SELECT name FROM `{$table1}` EXCEPT SELECT name FROM `{$table2}` ORDER BY name");

        $this->assertFalse($result);
    }

    public function testSelectExceptTriggersNoticeInNoticeMode(): void
    {
        $table1 = $this->uniqueTableName('users1');
        $table2 = $this->uniqueTableName('users2');
        ...
        try {
            ...
            $this->assertStringContainsString('EXCEPT', $capturedNotices[0]);
        } finally {
