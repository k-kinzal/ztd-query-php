...
final class HelpTest extends MySqlIntegrationTestCase
{
    public function testHelpThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("HELP 'SELECT'");
    }

    public function testHelpIsIgnoredInIgnoreMode(): void
    {
        $ztdPdo = $this->createZtdPdoWithConfig(new ZtdConfig(
            unsupportedBehavior: UnsupportedSqlBehavior::Ignore
        ));

        $result = $ztdPdo->query("HELP 'SELECT'");

        $this->assertFalse($result);
    }

    public function testHelpTriggersNoticeInNoticeMode(): void
    {
        $ztdPdo = $this->createZtdPdoWithConfig(new ZtdConfig(
        ...
        try {
            ...
            $this->assertStringContainsString('HELP', $capturedNotices[0]);
        } finally {
