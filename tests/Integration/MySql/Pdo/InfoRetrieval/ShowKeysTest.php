...
final class ShowKeysTest extends MySqlIntegrationTestCase
{
    public function testShowKeysThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("SHOW KEYS FROM `{$table}`");
    }

    public function testShowKeysIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("SHOW KEYS FROM `{$table}`");

        $this->assertFalse($result);
    }

    public function testShowKeysTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('SHOW KEYS', $capturedNotices[0]);
        } finally {
