...
final class DescTest extends MySqlIntegrationTestCase
{
    public function testDescThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("DESC `{$table}`");
    }

    public function testDescIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("DESC `{$table}`");

        $this->assertFalse($result);
    }

    public function testDescTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('DESC', $capturedNotices[0]);
        } finally {
