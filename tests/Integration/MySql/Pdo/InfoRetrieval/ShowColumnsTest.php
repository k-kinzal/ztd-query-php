...
final class ShowColumnsTest extends MySqlIntegrationTestCase
{
    public function testShowColumnsThrowsExceptionInExceptionMode(): void
    {
        ...
        $ztdPdo->query("SHOW COLUMNS FROM `{$table}`");
    }

    public function testShowColumnsIsIgnoredInIgnoreMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        $result = $ztdPdo->query("SHOW COLUMNS FROM `{$table}`");

        $this->assertFalse($result);
    }

    public function testShowColumnsTriggersNoticeInNoticeMode(): void
    {
        $table = $this->uniqueTableName('users');
        ...
        try {
            ...
            $this->assertStringContainsString('SHOW COLUMNS', $capturedNotices[0]);
        } finally {
