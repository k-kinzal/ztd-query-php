...
final class ReplaceDelayedTest extends MySqlIntegrationTestCase
{
    public function testReplaceDelayedIsSupported(): void
    {
        $table = $this->uniqueTableName('users');

        $this->rawPdo->exec("CREATE TABLE `{$table}` (id INT PRIMARY KEY, name VARCHAR(255))");

        $this->ztdPdo->exec("REPLACE DELAYED INTO `{$table}` (id, name) VALUES (1, 'Alice')");
