...
final class DeleteIgnoreTest extends MySqlIntegrationTestCase
{
    public function testDeleteIgnoreSyntaxIsSupported(): void
    {
        ...
        $affected = $this->ztdPdo->exec("DELETE IGNORE FROM `{$table}` WHERE id = 1");
    ...
    public function testDeleteIgnoreWithMultipleRows(): void
    {
        $table = $this->uniqueTableName('users');

        $this->rawPdo->exec("CREATE TABLE `{$table}` (id INT PRIMARY KEY, name VARCHAR(255))");
        $this->ztdPdo->exec("INSERT INTO `{$table}` VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')");

        $affected = $this->ztdPdo->exec("DELETE IGNORE FROM `{$table}` WHERE id <= 2");
